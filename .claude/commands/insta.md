...
- **⚠️ 미발행분 우선 소진 (필수)**: `run_log.md` 비고 + `hook_history.md`의 `미발행` 행·메모를 grep해 **아직 못 쓴 슬롯**이 있는지 확인. 있으면 **새 훅·카드를 짜기 전에 그 슬롯부터 다음 주 같은 요일에 재활용**한다(산출물은 `content-output/`에 이미 있음. 카드는 미발행이므로 사이클 미소진 = 재사용 가능).
  ...
  - **인스타 릴스가 미발행이면 같은 날 유튜브 쇼츠도 대개 함께 미발행**(첫 장면이 `insta/scene01.png` 복사본 = 자산 공유). 재활용 시 `youtube/video.mp4`도 같이 넘길 것. 유튜브 결번은 8주 게이트 표본을 훼손하므로 `hook_history.md` 메모에 결번일을 누적 기록
...
### 3. 파일 생성 순서
**인스타를 전부 끝낸 뒤 유튜브에 착수한다** (1~9 인스타 → 10 유튜브). 유튜브 첫 장면이 `insta/scene01.png`를 가져다 쓰므로 순서를 뒤집으면 안 됨.
...
10. **유튜브 쇼츠 생성** — 아래 "유튜브 쇼츠" 섹션 절차대로 프레임 생성 → `insta/scene01.png`를 `youtube/frames/s01-hook.png`로 복사 → `bash scripts/render-videos.sh youtube {날짜들}` → 각 `youtube/video.mp4` + `youtube/copy.txt`
...
- **인스타 자산 재활용**: 첫 장면(훅)은 `insta/scene01.png`를 **그대로 복사**해 `youtube/frames/s01-hook.png`로 사용. 새로 그리지 말 것
...
| 유형 | 요일 | 구성 | 길이 |
|---|---|---|---|
| **pick** (참여형) | 월·화·목·토·일 | 훅5s + 전환2s + 카드3×(리빌4s + 텍스트10s×2) + 아웃트로6s | 85초 |
| **single** (소개형) | 수·금 | 훅4s + 리빌5s + 텍스트10s×3 + 아웃트로6s | 45초 |
...
4. 인스타 훅 복사: `cp {날짜}/insta/scene01.png {날짜}/youtube/frames/s01-hook.png`
5. `bash scripts/render-videos.sh youtube {날짜들}` → `youtube/video.mp4`
6. 프레임 Read로 시각 검증 (자소 깨짐·오버플로우·카드 구도)
...
초기 채널이라 **85초/45초 단일 규격으로 고정**하고 A/B 하지 않는다. 표본이 작아 판독이 안 되고, 변수를 섞으면 "끝까지 보는 채널" 신호만 흐려짐. 8주 뒤 시청지속률 그래프를 보고 조정한다.
...
- [ ] 유튜브 첫 장면이 `insta/scene01.png` **복사본**인가 (새로 그려서 카드색 고정되지 않았는가)
