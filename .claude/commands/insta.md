...
주 7회 인스타 업로드 + 트위터 매일. **2026-05-04 업데이트**: 수요일 캐러셀 → 소개형 마이너로 변경. 캐러셀은 **토요일 1회만**.

| 요일 | 메인 콘텐츠 | 유형 | 카드 | 디자인 |
|---|---|---|---|---|
| 월 | shortform (3컷) | **소개형 메이저** | 메이저 1장 | NEW (`scene*_new.png`) |
| 화 | shortform (2컷) | 참여형 #2 | 공개 카드 3장(메이저) | 코스믹 참여형 |
| 수 | shortform (3컷) | **소개형 마이너** | 마이너 1장 | 기존 (`scene*.png`) |
...
| 일 | shortform (7컷) | **Sunday Tarot Preview** (주간 예고형) | 공개 카드 3장(메이저+마이너 혼합) | 코스믹 예고형 |

- 매일: twitter.txt 1개
- 참여형 릴스는 화·목 2회, reply_templates.txt 포함
- 소개형 마이너는 수·금 2회 (서로 다른 카드)
...
- 참여형 주제 후보: 다시 연락할까/마음속 1순위/이번 주 움직임/숨긴 진짜 감정/내가 먼저 연락해도 될까
...
- carousel 본문 (토): 마이너 포함 가능
- 같은 주 내 동일 카드 2회 메인 사용 금지

### 카운트 분리 (중요, 2026-05-04 업데이트)

같은 카드라도 **용도가 다르면 카운트 별도**. 메모리 `project_lovtaro_card_usage.md`는 아래 카테고리별로 분리해 추적한다:

- **월요일 소개형 메이저 메인** — 메이저 22장 사이클 별도
- **수·금 소개형 마이너 메인** — 마이너 56장 사이클 별도 (수+금 합쳐서 카운트, 같은 카드 재사용 금지)
- **토 캐러셀 커버 (메이저)** — 별도 사이클
- **토 캐러셀 본문 slide02~04 + 캐러셀 릴스 scene02~04 (마이너)** — 별도 사이클
- **참여형 reply 공개 카드 (메이저)** — 화·목 합쳐 별도 사이클
...
**How to apply:** 새 콘텐츠 카드 배정 시 해당 슬롯의 카운트만 확인. 예: 월요일 소개형 메이저 배정 시 "월요일 소개형 메이저 메인" 섹션만 보고 미사용 카드 선택. 다른 슬롯에서 사용된 카드여도 OK.

- **기록 대상(생성 후 반드시 업데이트, 슬롯별로)**:
  - 월요일 소개형 메이저 scene02 메인 카드
  - 수·금 소개형 마이너 scene02 메인 카드 (각각 표기)
  - 캐러셀 커버 (slide01) 메이저
  - 캐러셀 본문 slide02~04 마이너 (3장 전부)
  - 캐러셀 릴스 scene01/02~04 메인 카드 (커버 메이저, 본문 마이너 3장)
  - 참여형 scene01 공개 카드 3장 (뒷면이지만 reply에서 공개되므로 기록)
  - 참여형 scene02 공개 카드 (있으면)
  - Sunday Preview 3장
  - story 마이너
### 카드명 표기 (단일 소스)
...
5가지 공식 중 1개 이상에 부합해야 함. 단순 카드 설명형(예: "컵의 2 카드가 알려주는 메시지") 금지.
...
**주제 선정 기준**: "그 사람 마음이 궁금해요" 같은 막연한 질문보다 **"상대가 X 행동을 하는데 이게 무슨 뜻이죠?"** 처럼 구체 행동·상황을 출발점으로 잡을 것 (공식 2 우선).
...
### 훅·캡션 톤 (신규)
- [ ] 모든 첫 장 훅이 **5가지 훅 공식** 중 1개 이상에 부합하는가 (단순 카드 설명형 금지)
- [ ] 소개형 scene01에 **카드명 노출 없음** (사용자 고민 중심), 카드명·키워드는 scene02에서 공개되는가
- [ ] 참여형 scene01 훅에 **상황 한정 조건**(연락 기다리는 사람/다시 연락 올지 궁금한 등)이 포함되었는가
