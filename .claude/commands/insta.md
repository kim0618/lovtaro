...
- **⚠️ 미발행분 우선 소진 (필수)**: `run_log.md` 비고 + `hook_history.md`의 `미발행` 행·메모를 grep해 **아직 못 쓴 슬롯**이 있는지 확인. 있으면 **새 훅·카드를 짜기 전에 그 슬롯부터 다음 주 같은 요일에 재활용**한다(산출물은 `content-output/`에 이미 있음. 카드는 미발행이므로 사이클 미소진 = 재사용 가능).
  ...
  - ⚠️ **의도된 결번은 재활용 대상이 아니다**: 2026-09-10 목·09-12 토 인스타 릴스는 여행 특칙에 따른 계획 결번이다(아래 "주간 운영 구조" 참조). 미발행 큐에 올리지 말 것
...
1. 날짜별 폴더 구조 생성 (`{날짜}/insta/`. 토요일 `carousel/`는 2026-08-30 폐지로 만들지 않는다)
...
| 7위 | 토 | **1·2·3 참여형 릴스(2컷)만** (캐러셀 폐지 2026-08-30) | 참여형 릴스 | 혼합 3장 (메이저 최소 1장) | 코스믹 참여형 |
...
- **🚫 캐러셀 폐지 (2026-08-30 사용자 지시, "잘 안 보더라")**: 토요일은 **참여형 릴스 1개만**. `carousel/` 폴더 생성·`carousel/copy.txt` 작성·캐러셀 카드 사이클 배정을 전부 하지 않는다. 템플릿(`scripts/templates/carousel.mjs`)과 카드 사이클 기록은 rollback용으로 보존만 한다. ⚠️이 결정이 8/30 run_log에만 있고 이 배치표엔 반영이 안 돼 있었다(2026-09-04 반영). 아래 `carousel` 관련 섹션들은 보존 문서이지 주간 배치 지시가 아니다.
- **🧳 2026-09-07~09-13 주차 한정 특칙 (해외여행 9/9~9/13, 2026-09-04 사용자 지시)**: **목(9/10)·토(9/12) 인스타 릴스는 결번**한다.
  - **결번은 인스타 릴스만이다.** 유튜브 쇼츠(픽어카드는 답글 약속이 없다)·평일 스토리·트위터는 그대로 만들어 예약 발행한다. 두 날의 scene·해석 자산은 유튜브가 가져다 쓰므로 **생성은 정상 진행하고 인스타 업로드만 하지 않는다**.
  - 이유 ①목·토 둘 다 1·2·3 참여형이라 캡션의 "답글 달아드릴게요" 약속을 현지에서 이행할 수 없다. 약속 불이행은 미발행보다 나쁘다 ②토는 8/30 판정에서 슬롯 요인이 확정된 최저 칸이다(같은 엔진 프레임이 월 4,476 vs 토 958, 4.7배)
  - 9/7 월·9/8 화 참여형은 출발 전이라 정상 발행. 수·금 소개형과 일 Sunday Preview는 답글 약속 문구가 없어 그대로 유지
  - 이 주차 참여형 4회 → 2회, 답글 부담 15개 → 9개
  - ⚠️ **이 결번은 의도된 것이므로 "미발행 우선 소진" 재활용 큐에 넣지 않는다.** 다음 실행(9/14~ 주차)이 9/17·9/19에 자동 재활용하려 들면 안 된다. 카드는 유튜브 발행으로 소진 처리한다(2026-08-26 인스타·유튜브 분리 발행 선례와 동일)
  - **9/14 복귀 이후 주차는 표준 배치로 원복**하고 이 특칙을 적용하지 않는다
...
- **통합 위치 (매주 2곳)**: 참여형 **2회**(월·화·목 중 2개, 해당 요일 없으면 그 주 참여형 중 2개)의 `[인스타 첫 댓글]`. ~~토 캐러셀~~은 2026-08-30 캐러셀 폐지로 제외
