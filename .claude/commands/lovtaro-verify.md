...
### ⚠️ P의 "중복 없음"은 의미 중복의 무죄 증명이 아니다 (2026-08-24 신설, 실제 사고)

**어휘를 바꾸면 N-gram은 얼마든지 내려가지만 문장 골격은 남는다.** `priestess-love-meaning` FAQ1 보강(8/24)이 카드 사전 `upright.love`의 "상대방 자리" 문단과 **4비트 1:1 대응**이었는데(말 아낌 / 표현 없다고 감정 없는 게 아님 / 안에서 정리 중 / 재촉 말고 여유), 최장 연속 겹침이 **8자**라 P가 조용히 통과했다. 스킬(`lovtaro-guide` 4-9)의 "카드 사전 `love` 문장과 60% 이상 겹치는 문장 금지"는 **정규식으로 잡히지 않는 항목**이다.

더 나쁜 건 **그 문장이 원래 없었고 이번 보강에서 새로 들어갔다**는 점이다([[feedback_fix_introduces_new_defects]]). "심화하겠다"고 손댄 결과가 사전 재서술을 만들었다.

**카드 해석 글의 FAQ·섹션을 고쳤으면, 그 카드 사전 블록에서 같은 주제 문단을 찾아 비트 단위로 나란히 적어본다.**

```
[사전]  말 아낌 → 표현 없다고 감정 없는 게 아님 → 안에서 정리 중 → 재촉 말고 여유
[내 글] 말 아낌 → 표현 없다고 감정 없는 게 아님 → 안에서 정리 중 → 캐묻지 말고 시간   ← 4/4 대응 = 재서술
```

**비트가 절반 이상 대응하면 N-gram 수치와 무관하게 재작성한다.** 고치는 방향은 어휘 치환이 아니라 **역할 분리**다. 위 사고는 축을 사전의 "상대는 말을 아끼는 중"(상태 안내)에서 **"상대 안에서도 아직 답이 문장으로 만들어지지 않은 단계"**(그 리딩에서 무엇을 기대하면 안 되는지)로 옮겨 해소했고, 재측정 최장 겹침이 8자 → 5자로 떨어졌다.
