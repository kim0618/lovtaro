...
2. **전통 해몽 근거 확인** - 그 꿈의 표준 통념을 파악. 불확실하면 `WebSearch`로 확인. (예: 뱀=재물·이성운, 이빨=가족·구설, 태몽=새 생명·변화)
   ...
   **전통 근거층 점수를 세고 큐에 적는다.** 5갈래면 몇 갈래에 실제 사전 근거가 붙었는지(n/5). 기준선: accident 5/5, giving-birth 5/5, drinking 4/5, bath 3/5, **military 초판 1/5(실패)**. 3/5 미만이면 발행하지 말고 주제를 바꾸거나 갈래를 재구성한다.
