...
- **큐 보충(전환) 절차**: 초기 1~3주는 아래 부트스트랩 큐(연애 직결도 순)를 소진. 데이터가 쌓이면(3주차~) 가이드와 동일하게 **네이버 서치어드바이저 "검색 키워드/웹문서 TOP30" 노출 역산**으로 재정렬. 우리 `/dream/*`가 받는 노출 큰 미발행 꿈부터 끌어올린다. 큐 거의 소진되면 사용자에게 네이버 화면 요청.
