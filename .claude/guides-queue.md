...
- **큐 보충(재정렬) 절차** (2026-06-01 확정): 네이버 노출 데이터는 GA4/구글 서치콘솔에 안 잡혀 자동 조회 불가. API 연동도 안 함(수동 유지 결정). 큐가 거의 소진되면 **사용자에게 네이버 서치어드바이저 "검색 키워드/웹문서 TOP30" 화면을 요청**해서, 노출 큰 미발행 카드 순으로 다음 큐를 짠다. 보조로 GA4 네이버 랜딩 페이지(m.search.naver.com)도 참고 가능.
