...
const outputDir = resolve(rootDir, 'content-output/2026-09-18_fri/insta')
const W = 1080, H = 1920
...
const PORTRAIT_OVERRIDE = {
  ...
  'nine-of-cups': 'public/images/mcards/cups/Nine of Cups.png',
  // 신규 카드 사용 시 여기 추가
}
...
async function scene01() {
  ...
  const cardSlug = 'nine-of-cups'
  const hookLineTop = '만나기도 전에,'
  const accentBefore = '상상 속에서 벌써 '
  const accentWord = '백 번은'
  const accentAfter = ' 사귄 사람'
...
async function scene02() {
  ...
  const cardRaw = await loadCard('nine-of-cups', cardW, cardH)
  ...
  const svg = `<svg xmlns="http://www.w3.org/2000/svg" width="${W}" height="${H}">
    ...
    <g filter="url(#softGlow)">
      <text x="540" y="${nameKrY}" text-anchor="middle" font-family="'Noto Sans KR','Apple SD Gothic Neo',NanumSquare,sans-serif" font-size="58" fill="#F4F8FF" font-weight="300" letter-spacing="4">컵의 9</text>
      <text x="540" y="${nameEnY}" text-anchor="middle" font-family="Georgia, serif" font-style="italic" font-size="30" fill="rgba(232,212,139,0.88)" letter-spacing="1">Nine of Cups</text>
    </g>

    <text x="540" y="${kwY}" text-anchor="middle" font-family="'Noto Sans KR','Apple SD Gothic Neo',NanumSquare,sans-serif" font-size="26" fill="rgba(232,212,139,0.72)" letter-spacing="4" font-weight="300">소원 성취 · 만족 · 행복</text>
...
async function scene03() {
  ...
  const cardImg = await loadCard('nine-of-cups', cardW, cardH)
  ...
  const svg = `<svg xmlns="http://www.w3.org/2000/svg" width="${W}" height="${H}">
    ...
    <g filter="url(#softGlow)">
      <text x="540" y="${headerY}" text-anchor="middle" font-family="'Noto Sans KR','Apple SD Gothic Neo',NanumSquare,sans-serif" font-size="40" fill="#F4F8FF" font-weight="300" letter-spacing="6">앞서 달려가는 마음</text>
    </g>
    ...
    <text x="540" y="${nameKrY}" text-anchor="middle" font-family="'Noto Sans KR','Apple SD Gothic Neo',NanumSquare,sans-serif" font-size="34" fill="#F4F8FF" font-weight="300" letter-spacing="4">컵의 9</text>
    <text x="540" y="${nameEnY}" text-anchor="middle" font-family="Georgia, serif" font-style="italic" font-size="20" fill="rgba(232,212,139,0.8)" letter-spacing="1">Nine of Cups</text>

    <text x="540" y="${kwY}" text-anchor="middle" font-family="'Noto Sans KR','Apple SD Gothic Neo',NanumSquare,sans-serif" font-size="22" fill="rgba(232,212,139,0.6)" letter-spacing="4" font-weight="300">소원 성취 · 만족 · 행복</text>

    <g filter="url(#softGlow)">
      <text x="540" y="${interpY}" text-anchor="middle" font-family="'Noto Sans KR','Apple SD Gothic Neo',NanumSquare,sans-serif" font-size="34" fill="#F4F8FF" font-weight="300" letter-spacing="1">상상이 커지는 건,</text>
      <text x="540" y="${interpY + 58}" text-anchor="middle" font-family="'Noto Sans KR','Apple SD Gothic Neo',NanumSquare,sans-serif" font-size="34" fill="#F4F8FF" font-weight="300" letter-spacing="1">그만큼 기대하고 있어서예요.</text>
      <text x="540" y="${interpY + 116}" text-anchor="middle" font-family="'Noto Sans KR','Apple SD Gothic Neo',NanumSquare,sans-serif" font-size="34" fill="#F4F8FF" font-weight="300" letter-spacing="1">진짜 설렘은 만나서 확인해도 늦지 않아요.</text>
    </g>
...
async function main() {
  console.log('=== 2026-09-18_fri 소개형 마이너 (Nine of Cups) ===')
