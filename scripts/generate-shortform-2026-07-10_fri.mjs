/**
 * 소개형 마이너 shortform 2026-07-10_fri (Page of Wands)
 * 훅: 아직 아무 사이도 아닌데, / 왜 자꾸 이 사람이 신경 쓰일까?
 */
...
const outputDir = resolve(rootDir, 'content-output/2026-07-10_fri/shortform')
const W = 1080, H = 1920

const PORTRAIT_OVERRIDE = {
  'page-of-wands': 'public/images/mcards/wands/Page of Wands.png',
}
...
async function scene01() {
  const cardSlug = 'page-of-wands'
  const hookLineTop = '아직 아무 사이도 아닌데,'
  const accentBefore = '왜 자꾸 '
  const accentWord = '이 사람이 신경 쓰일까'
  const accentAfter = '?'
...
async function scene02() {
  ...
  const cardRaw = await loadCard('page-of-wands', cardW, cardH)
  ...
  const svg = `<svg xmlns="http://www.w3.org/2000/svg" width="${W}" height="${H}">
    ...
    <g filter="url(#softGlow)">
      <text x="540" y="${nameKrY}" text-anchor="middle" font-family="'Noto Sans KR','Apple SD Gothic Neo',NanumSquare,sans-serif" font-size="58" fill="#F4F8FF" font-weight="300" letter-spacing="4">완드의 페이지</text>
      <text x="540" y="${nameEnY}" text-anchor="middle" font-family="Georgia, serif" font-style="italic" font-size="30" fill="rgba(232,212,139,0.88)" letter-spacing="1">Page of Wands</text>
    </g>

    <text x="540" y="${kwY}" text-anchor="middle" font-family="'Noto Sans KR','Apple SD Gothic Neo',NanumSquare,sans-serif" font-size="26" fill="rgba(232,212,139,0.72)" letter-spacing="4" font-weight="300">열정 · 탐험 · 새로운 아이디어</text>
...
async function scene03() {
  ...
  const cardImg = await loadCard('page-of-wands', cardW, cardH)
  ...
  const svg = `<svg xmlns="http://www.w3.org/2000/svg" width="${W}" height="${H}">
    ...
    <g filter="url(#softGlow)">
      <text x="${W / 2}" y="${headerY}" text-anchor="middle" font-family="'Noto Sans KR','Apple SD Gothic Neo',NanumSquare,sans-serif" font-size="40" fill="#F4F8FF" font-weight="300" letter-spacing="6">아직 이름 붙지 않은 설렘을 들여다보면</text>
    </g>
    ...
    <text x="${W / 2}" y="${nameKrY}" text-anchor="middle" font-family="'Noto Sans KR','Apple SD Gothic Neo',NanumSquare,sans-serif" font-size="34" fill="#F4F8FF" font-weight="300" letter-spacing="4">완드의 페이지</text>
    <text x="${W / 2}" y="${nameEnY}" text-anchor="middle" font-family="Georgia, serif" font-style="italic" font-size="20" fill="rgba(232,212,139,0.8)" letter-spacing="1">Page of Wands</text>

    <text x="${W / 2}" y="${kwY}" text-anchor="middle" font-family="'Noto Sans KR','Apple SD Gothic Neo',NanumSquare,sans-serif" font-size="22" fill="rgba(232,212,139,0.6)" letter-spacing="4" font-weight="300">열정 · 탐험 · 새로운 아이디어</text>

    <g filter="url(#softGlow)">
      <text x="${W / 2}" y="${interpY}" text-anchor="middle" font-family="'Noto Sans KR','Apple SD Gothic Neo',NanumSquare,sans-serif" font-size="34" fill="#F4F8FF" font-weight="300" letter-spacing="1">아무것도 시작되지 않았는데 자꾸 눈이 가는 건,</text>
      <text x="${W / 2}" y="${interpY + 58}" text-anchor="middle" font-family="'Noto Sans KR','Apple SD Gothic Neo',NanumSquare,sans-serif" font-size="34" fill="#F4F8FF" font-weight="300" letter-spacing="1">마음이 새로운 불씨를 먼저 알아챘다는 신호일 수 있어요.</text>
      <text x="${W / 2}" y="${interpY + 116}" text-anchor="middle" font-family="'Noto Sans KR','Apple SD Gothic Neo',NanumSquare,sans-serif" font-size="34" fill="#F4F8FF" font-weight="300" letter-spacing="1">조급해하지 않아도, 그 관심을 가볍게 따라가 봐도 좋아요.</text>
    </g>
...
async function main() {
  console.log('=== 7/10 금 소개형 마이너 (Page of Wands) ===')
  mkdirSync(outputDir, { recursive: true })
  await scene01()
  await scene02()
  await scene03()
