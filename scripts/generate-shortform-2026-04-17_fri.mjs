/**
 * 2026-04-17 금요일 shortform (릴스) 이미지 생성
 * - 캐러셀과 같은 내용, 1080x1920 레이아웃
 * - scene01: 커버 — Judgement 풀사이즈 + 하단 페이드 + 훅
 * - scene02~04: 카드 풀배경 + 해석 텍스트
 * - scene05: CTA — Star 카드 + 저장/공유
 *
...
async function scene01() {
  ...
  <ellipse cx="540" cy="650" rx="500" ry="600" fill="url(#glow)"/>
  ${stars}
  ...
  if (masked) {
    ...
    <rect x="0" y="1050" width="${W}" height="870" fill="url(#bf)"/>
    <text x="540" y="1560" font-family="sans-serif" font-size="48" font-weight="300" fill="#F4F8FF" text-anchor="middle">읽씹했던 그 사람,</text>
    <text x="540" y="1625" font-family="sans-serif" font-size="48" font-weight="300" fill="#F4F8FF" text-anchor="middle">다시 연락이 올까?</text>
    <text x="540" y="1780" font-family="sans-serif" font-size="22" fill="rgba(180,170,220,0.45)" text-anchor="middle">스와이프해서 확인하세요 →</text>
    </svg>`
...
// ── scene02~04: 카드 풀배경 + 텍스트 오버레이 ──
async function contentScene(cardSlug, nameEn, subtitle, bodyText, index, filename) {
  ...
  const totalTextH = engNameSize + 20 + subtitleSize + 60 + bodyTotalH
  const startY = Math.round((H - totalTextH) / 2)
  const engNameY = startY + engNameSize
  const subtitleY = engNameY + 55
  const lineY1 = engNameY - engNameSize - 12
  const lineY2 = subtitleY + 20

  let curY = subtitleY + 70
  ...
  <text x="1030" y="60" font-family="sans-serif" font-size="20" fill="rgba(180,170,230,0.5)" text-anchor="end">${index}/3</text>
...
async function scene05() {
  ...
  const saveIcon = `<g transform="translate(430, 1150)">
  ...
  const shareIcon = `<g transform="translate(570, 1150)">
  ...
  <rect width="${W}" height="${H}" fill="url(#bg)"/>
  <ellipse cx="540" cy="600" rx="450" ry="500" fill="url(#glow)"/>
  ...
  ${saveIcon}
  ${shareIcon}
  <text x="540" y="1320" font-family="sans-serif" font-size="38" font-weight="300" fill="#F4F8FF" text-anchor="middle">저장해두고</text>
  <text x="540" y="1375" font-family="sans-serif" font-size="26" fill="rgba(210,200,250,0.65)" text-anchor="middle">연락운이 궁금할 때 다시 꺼내보세요</text>
  <line x1="380" y1="1420" x2="700" y2="1420" stroke="rgba(180,170,230,0.12)" stroke-width="1"/>
  <text x="540" y="1470" font-family="sans-serif" font-size="24" fill="rgba(200,190,240,0.5)" text-anchor="middle">읽씹 고민하는 친구가 있다면</text>
  <text x="540" y="1505" font-family="sans-serif" font-size="24" fill="rgba(200,190,240,0.5)" text-anchor="middle">가볍게 공유해보세요</text>
  </svg>`
...
async function main() {
  console.log('=== 2026-04-17 shortform (릴스) 이미지 생성 ===')
