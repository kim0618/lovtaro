/**
 * TEMPLATE: story (1장, 1080×1920, 마이너 아르카나 전용)
 * - 카드 680×930, cardTop 170 + 한국어 훅 + 영어 카드명 + 키워드
 * - 카드 경로: public/images/mcards/{suit}/{Card Name}.png
 *
 * 사용: scripts/templates/story.mjs 를 scripts/generate-story-{YYYY-MM-DD_day}.mjs 로 복사.
 * 스토리는 이미지 1장만 생성 - copy.txt 만들지 말 것.
 */
...
const outputDir = resolve(rootDir, 'content-output/2026-04-20_mon/story')
const W = 1080, H = 1920
...
async function main() {
  console.log('=== 2026-04-20 story 이미지 생성 ===')
  ...
  const img = await loadCard('cups', 'Ten of Cups', cW, cH)
  if (!img) { console.error('❌ Ten of Cups.png 없음'); process.exit(1) }
  ...
  const hookLines = ['함께 쌓여가는 따뜻한 시간', '이번 주 감정의 결이 좋아져요']
  const keywords = ['행복', '조화', '안정된 마음']
  ...
  <text x="540" y="${engNameY}" font-family="Georgia, serif" font-size="22" font-style="italic" fill="rgba(200,180,140,0.65)" text-anchor="middle">Ten of Cups</text>
  <g filter="url(#tg)">
    ${hookSvg}
  </g>
