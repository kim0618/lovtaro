/**
 * 소개형 마이너 shortform 2026-05-29_fri (Two of Pentacles)
 * 훅: 여러 곳에 마음 둔 사람, / 왜 가장 중요한 답은 늘 미룰까?
 */
...
const outputDir = resolve(rootDir, 'content-output/2026-05-29_fri/shortform')
const W = 1080, H = 1920
...
async function scene01() {
  ...
  const hookLineTop = '여러 곳에 마음 둔 사람,'
  const accentBefore = '왜 가장 중요한 답은 늘 '
  const accentWord = '미룰까'
  const accentAfter = '?'
...
async function scene02() {
  ...
  const svg = `<svg xmlns="http://www.w3.org/2000/svg" width="${W}" height="${H}">
    ...
    <text x="540" y="${kwY}" text-anchor="middle" font-family="'Noto Sans KR','Apple SD Gothic Neo',NanumSquare,sans-serif" font-size="26" fill="rgba(232,212,139,0.72)" letter-spacing="4" font-weight="300">균형 · 조율 · 유연함</text>
...
async function scene03() {
  ...
  const svg = `<svg xmlns="http://www.w3.org/2000/svg" width="${W}" height="${H}">
    ...
    <g filter="url(#softGlow)">
      <text x="${W / 2}" y="${headerY}" text-anchor="middle" font-family="'Noto Sans KR','Apple SD Gothic Neo',NanumSquare,sans-serif" font-size="40" fill="#F4F8FF" font-weight="300" letter-spacing="6">균형을 잡느라 멈춰 있는 마음</text>
    </g>
    ...
    <text x="${W / 2}" y="${kwY}" text-anchor="middle" font-family="'Noto Sans KR','Apple SD Gothic Neo',NanumSquare,sans-serif" font-size="22" fill="rgba(232,212,139,0.6)" letter-spacing="4" font-weight="300">균형 · 조율 · 유연함</text>

    <g filter="url(#softGlow)">
      <text x="${W / 2}" y="${interpY}" text-anchor="middle" font-family="'Noto Sans KR','Apple SD Gothic Neo',NanumSquare,sans-serif" font-size="34" fill="#F4F8FF" font-weight="300" letter-spacing="1">여러 결을 한꺼번에 잡고 있어요.</text>
      <text x="${W / 2}" y="${interpY + 58}" text-anchor="middle" font-family="'Noto Sans KR','Apple SD Gothic Neo',NanumSquare,sans-serif" font-size="34" fill="#F4F8FF" font-weight="300" letter-spacing="1">균형을 지키느라 답이 자꾸 미뤄져요.</text>
      <text x="${W / 2}" y="${interpY + 116}" text-anchor="middle" font-family="'Noto Sans KR','Apple SD Gothic Neo',NanumSquare,sans-serif" font-size="34" fill="#F4F8FF" font-weight="300" letter-spacing="1">우선순위를 다시 정리할 시기예요.</text>
    </g>
...
async function main() {
  console.log('=== 2026-05-29 소개형 마이너 shortform (Two of Pentacles) ===')
  mkdirSync(outputDir, { recursive: true })
  await scene01()
  await scene02()
  await scene03()
