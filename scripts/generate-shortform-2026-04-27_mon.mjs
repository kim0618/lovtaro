...
const outputDir = resolve(rootDir, 'content-output/2026-04-27_mon/shortform')
...
async function scene01() {
  ...
  const overlaySvg = `<svg xmlns="http://www.w3.org/2000/svg" width="${W}" height="${H}">
    ...
    <g filter="url(#textShadow)">
      <text x="540" y="360" text-anchor="middle" class="hook" fill="#FFFFFF">감정이 자꾸 <tspan class="hook-accent">흔들릴</tspan> 때</text>
      <text x="540" y="442" text-anchor="middle" class="hook" fill="#FFFFFF">어떻게 마음을 잡아야 할까?</text>
    </g>

    <g filter="url(#textShadow)">
      <text x="540" y="1640" text-anchor="middle" class="cta" fill="#D4AE68">카드가 조용히 알려줄 거예요</text>
    </g>
...
async function scene02() {
  ...
  const cardSlug = 'strength'
  const cardNameKr = '힘'
  const cardNameEn = 'Strength'
  const keywords = '인내 · 다정한 단단함 · 감정 다잡기'
...
async function scene03() {
  ...
  const overlaySvg = `<svg xmlns="http://www.w3.org/2000/svg" width="${W}" height="${H}">
    ...
    <g filter="url(#textShadow)">
      <text x="540" y="220" text-anchor="middle" class="header" fill="#FFFFFF">부드럽게 <tspan class="hook-accent">다잡는</tspan> 마음</text>
    </g>
    ...
    <g filter="url(#textShadow)">
      <text x="540" y="${nameKrY}" text-anchor="middle" class="card-name-kr-sm" fill="#FFFFFF">힘</text>
      <text x="540" y="${nameEnY}" text-anchor="middle" class="card-name-en-sm" fill="#E8C877">Strength</text>
    </g>

    <g filter="url(#textShadow)">
      <text x="540" y="${kwY}" text-anchor="middle" class="keywords-sm" fill="#E8C877">인내 · 다정함 · 감정 다잡기</text>
    </g>

    <g filter="url(#textShadow)">
      <text x="540" y="${interpY}"        text-anchor="middle" class="interp" fill="#FFFFFF">강함은 누르는 힘이 아니에요.</text>
      <text x="540" y="${interpY + 70}"  text-anchor="middle" class="interp" fill="#FFFFFF">감정 옆에 <tspan class="hook-accent">조용히</tspan> 머물며,</text>
      <text x="540" y="${interpY + 140}" text-anchor="middle" class="interp" fill="#FFFFFF">결을 다정히 풀어주는 힘이에요.</text>
    </g>
...
async function main() {
  console.log('=== 2026-04-27 mon - 소개형 메이저 NEW (Strength) ===')
