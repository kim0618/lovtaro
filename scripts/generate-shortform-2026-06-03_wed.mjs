/**
 * 소개형 마이너 shortform 2026-06-03_wed (Knight of Cups)
 * 훅: 다정한 말은 다 하면서, / 왜 관계는 늘 제자리일까?
 */
...
const outputDir = resolve(rootDir, 'content-output/2026-06-03_wed/shortform')
const W = 1080, H = 1920

const PORTRAIT_OVERRIDE = {
  'knight-of-cups': 'public/images/mcards/cups/Knight of Cups.png',
}
...
async function scene01() {
  const cardSlug = 'knight-of-cups'
  const hookLineTop = '다정한 말은 다 하면서,'
  const accentBefore = '왜 관계는 늘 '
  const accentWord = '제자리'
  const accentAfter = '일까?'
...
async function scene02() {
  ...
  const cardRaw = await loadCard('knight-of-cups', cardW, cardH)
  ...
  const svg = `<svg xmlns="http://www.w3.org/2000/svg" width="${W}" height="${H}">
    ...
    <g filter="url(#softGlow)">
      <text x="540" y="${nameKrY}" text-anchor="middle" font-family="'Noto Sans KR','Apple SD Gothic Neo',NanumSquare,sans-serif" font-size="58" fill="#F4F8FF" font-weight="300" letter-spacing="4">컵의 나이트</text>
      <text x="540" y="${nameEnY}" text-anchor="middle" font-family="Georgia, serif" font-style="italic" font-size="30" fill="rgba(232,212,139,0.88)" letter-spacing="1">Knight of Cups</text>
    </g>

    <text x="540" y="${kwY}" text-anchor="middle" font-family="'Noto Sans KR','Apple SD Gothic Neo',NanumSquare,sans-serif" font-size="26" fill="rgba(232,212,139,0.72)" letter-spacing="4" font-weight="300">로맨틱 · 감정적 행동 · 구애</text>
...
async function scene03() {
  ...
  const cardImg = await loadCard('knight-of-cups', cardW, cardH)
  ...
  const svg = `<svg xmlns="http://www.w3.org/2000/svg" width="${W}" height="${H}">
    ...
    <g filter="url(#softGlow)">
      <text x="${W / 2}" y="${headerY}" text-anchor="middle" font-family="'Noto Sans KR','Apple SD Gothic Neo',NanumSquare,sans-serif" font-size="40" fill="#F4F8FF" font-weight="300" letter-spacing="6">마음은 보내면서 한 걸음이 느린 사람</text>
    </g>
    ...
    <text x="${W / 2}" y="${nameKrY}" text-anchor="middle" font-family="'Noto Sans KR','Apple SD Gothic Neo',NanumSquare,sans-serif" font-size="34" fill="#F4F8FF" font-weight="300" letter-spacing="4">컵의 나이트</text>
    <text x="${W / 2}" y="${nameEnY}" text-anchor="middle" font-family="Georgia, serif" font-style="italic" font-size="20" fill="rgba(232,212,139,0.8)" letter-spacing="1">Knight of Cups</text>

    <text x="${W / 2}" y="${kwY}" text-anchor="middle" font-family="'Noto Sans KR','Apple SD Gothic Neo',NanumSquare,sans-serif" font-size="22" fill="rgba(232,212,139,0.6)" letter-spacing="4" font-weight="300">로맨틱 · 감정적 행동 · 구애</text>

    <g filter="url(#softGlow)">
      <text x="${W / 2}" y="${interpY}" text-anchor="middle" font-family="'Noto Sans KR','Apple SD Gothic Neo',NanumSquare,sans-serif" font-size="34" fill="#F4F8FF" font-weight="300" letter-spacing="1">마음을 표현하는 데는 망설임이 없어요.</text>
      <text x="${W / 2}" y="${interpY + 58}" text-anchor="middle" font-family="'Noto Sans KR','Apple SD Gothic Neo',NanumSquare,sans-serif" font-size="34" fill="#F4F8FF" font-weight="300" letter-spacing="1">다만 다음 단계로는 쉽게 움직이지 않아요.</text>
      <text x="${W / 2}" y="${interpY + 116}" text-anchor="middle" font-family="'Noto Sans KR','Apple SD Gothic Neo',NanumSquare,sans-serif" font-size="34" fill="#F4F8FF" font-weight="300" letter-spacing="1">먼저 한 걸음 정해주면 흐름이 달라져요.</text>
    </g>
...
async function main() {
  console.log('=== 2026-06-03 소개형 마이너 shortform (Knight of Cups) ===')
  mkdirSync(outputDir, { recursive: true })
  await scene01()
  await scene02()
  await scene03()
