...
const outputDir = resolve(rootDir, 'content-output/2026-08-15_sat/carousel')
...
async function slide01() {
  ...
  const img = await loadCard('chariot', cW, cH)
  ...
  if (masked) {
    ...
    <text x="540" y="1085" font-family="sans-serif" font-size="44" font-weight="300" fill="#F4F8FF" text-anchor="middle">답장이 늦어질 때</text>
    <text x="540" y="1145" font-family="sans-serif" font-size="44" font-weight="300" fill="#F4F8FF" text-anchor="middle">절대 하면 안 되는</text>
    <text x="540" y="1205" font-family="sans-serif" font-size="44" font-weight="300" fill="#F4F8FF" text-anchor="middle">3가지</text>
...
async function main() {
  console.log('=== 2026-08-15 carousel 이미지 생성 (The Chariot) ===')
  mkdirSync(outputDir, { recursive: true })

  await slide01()

  await contentSlide('seven-of-wands',
    'Seven of Wands', '이유를 캐묻지 마세요',
    '왜 답이 늦었는지 따지는 순간\n상대는 방어부터 하게 돼요\n\n설명을 받아내는 것보다\n분위기를 지키는 게 먼저예요\n\n키워드: 방어 · 인내 · 거리',
    1, 'slide02.png')

  await contentSlide('four-of-swords',
    'Four of Swords', '연속으로 보내지 마세요',
    '답이 없을 때 덧붙이는 말은\n마음이 아니라 조급함으로 읽혀요\n\n한 박자 쉬는 동안\n상대에게도 돌아올 자리가 생겨요\n\n키워드: 휴식 · 회복 · 여백',
    2, 'slide03.png')

  await contentSlide('nine-of-pentacles',
    'Nine of Pentacles', '내 하루를 비워두지 마세요',
    '답장만 기다리는 하루가 쌓이면\n관계의 무게중심이 한쪽으로 기울어요\n\n내 자리가 단단할수록\n기다림도 조급해지지 않아요\n\n키워드: 독립 · 자기 완성 · 여유',
    3, 'slide04.png')
