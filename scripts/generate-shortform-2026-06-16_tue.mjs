/**
 * 참여형 shortform (토 릴스) 2026-06-06_sat (마법사 / Six of Cups / 연인)
 * 훅: 이번 주말 안에, / 그 사람이 먼저 연락해올까요?
 */
...
const outputDir = resolve(rootDir, 'content-output/2026-06-16_tue/shortform')
const W = 1080, H = 1920
...
async function generateScene01() {
  ...
  // 부채꼴 펼침 (2026-06-07 레버3: 일렬→부채꼴, 피드 썸네일 차별화)
  const cx2 = 540, cx1 = 340, cx3 = 740
  const cyMid = 945, cySide = 1000
  const cardY = cyMid
  const numberY = 1305
  ...
  const svg = `<svg xmlns="http://www.w3.org/2000/svg" width="${W}" height="${H}">
    ...
    ${participationBody(116)}

    <ellipse cx="540" cy="${cardY}" rx="500" ry="380" fill="url(#cardAreaGlow)" filter="url(#glowBlur)"/>

    <g filter="url(#softGlow)">
      <text x="540" y="380" text-anchor="middle" font-family="'Noto Sans KR','Apple SD Gothic Neo',NanumSquare,sans-serif" font-size="48" fill="#F4F8FF" letter-spacing="3" font-weight="300">아직 못 보낸 이 마음,</text>
      <text x="540" y="455" text-anchor="middle" font-family="'Noto Sans KR','Apple SD Gothic Neo',NanumSquare,sans-serif" font-size="48" fill="#F4F8FF" letter-spacing="3" font-weight="300">미련일까 미안함일까?</text>
    </g>

    <ellipse cx="${cx1}" cy="${cySide}" rx="${cardPixelW * 0.8}" ry="${cardPixelH * 0.55}" fill="url(#colorCardGlow_${s1})" filter="url(#glowBlur)"/>
    <ellipse cx="${cx2}" cy="${cyMid}" rx="${cardPixelW * 0.8}" ry="${cardPixelH * 0.55}" fill="url(#colorCardGlow_${s2})" filter="url(#glowBlur)"/>
    <ellipse cx="${cx3}" cy="${cySide}" rx="${cardPixelW * 0.8}" ry="${cardPixelH * 0.55}" fill="url(#colorCardGlow_${s3})" filter="url(#glowBlur)"/>

    <g filter="url(#cardShadow)">
      <g transform="rotate(-11 ${cx1} ${cySide})">${colorCardBackSvg(cx1, cySide, cardScale, s1)}</g>
      <g transform="rotate(11 ${cx3} ${cySide})">${colorCardBackSvg(cx3, cySide, cardScale, s3)}</g>
      ${colorCardBackSvg(cx2, cyMid, cardScale, s2)}
    </g>
...
async function generateScene02() {
  const svg = `<svg xmlns="http://www.w3.org/2000/svg" width="${W}" height="${H}">
    ...
    <line x1="240" y1="1300" x2="840" y2="1300" stroke="#e8d48b" stroke-width="2" opacity="0.8"/>
    ...
    <text x="540" y="1240" text-anchor="middle" font-family="sans-serif" font-size="32" fill="rgba(232,212,139,0.8)" letter-spacing="2" font-weight="300">마음 가는 사람에게 공유해보세요</text>

    <g opacity="0.85" filter="url(#softGlow)">
      <circle cx="480" cy="1400" r="5" fill="#e8d48b"/>
      <circle cx="540" cy="1400" r="5" fill="#e8d48b"/>
      <circle cx="600" cy="1400" r="5" fill="#e8d48b"/>
    </g>
...
async function main() {
  console.log('=== 2026-06-09 참여형 릴스 shortform 이미지 생성 ===')
