...
const outputDir = resolve(rootDir, 'content-output/2026-09-16_wed/insta')
const W = 1080, H = 1920
...
async function scene01() {
  ...
  const hookLineTop = '누구에게나 다정한 사람인 걸 알면서,'
  const accentBefore = '왜 나한테만은 '
  const accentWord = '다르길'
  const accentAfter = ' 바랄까?'
  ...
  const cardBg = await sharp(cardRaw)
    .blur(4)
    .modulate({ saturation: 1.18, brightness: 1.20 })
    .toBuffer()
...
async function scene03() {
  ...
  const svg = `<svg xmlns="http://www.w3.org/2000/svg" width="${W}" height="${H}">
    ...
    <g filter="url(#softGlow)">
      <text x="540" y="${headerY}" text-anchor="middle" font-family="'Noto Sans KR','Apple SD Gothic Neo',NanumSquare,sans-serif" font-size="40" fill="#F4F8FF" font-weight="300" letter-spacing="6">나에게만 특별하길 바라는 마음</text>
    </g>
    ...
    <text x="540" y="${nameKrY}" text-anchor="middle" font-family="'Noto Sans KR','Apple SD Gothic Neo',NanumSquare,sans-serif" font-size="34" fill="#F4F8FF" font-weight="300" letter-spacing="4">컵의 나이트</text>
    <text x="540" y="${nameEnY}" text-anchor="middle" font-family="Georgia, serif" font-style="italic" font-size="20" fill="rgba(232,212,139,0.8)" letter-spacing="1">Knight of Cups</text>

    <text x="540" y="${kwY}" text-anchor="middle" font-family="'Noto Sans KR','Apple SD Gothic Neo',NanumSquare,sans-serif" font-size="22" fill="rgba(232,212,139,0.6)" letter-spacing="4" font-weight="300">로맨틱 · 감정적 행동 · 구애</text>

    <g filter="url(#softGlow)">
      <text x="540" y="${interpY}" text-anchor="middle" font-family="'Noto Sans KR','Apple SD Gothic Neo',NanumSquare,sans-serif" font-size="34" fill="#F4F8FF" font-weight="300" letter-spacing="1">다정함이 모두의 것이어도,</text>
      <text x="540" y="${interpY + 58}" text-anchor="middle" font-family="'Noto Sans KR','Apple SD Gothic Neo',NanumSquare,sans-serif" font-size="34" fill="#F4F8FF" font-weight="300" letter-spacing="1">내가 느낀 설렘까지</text>
      <text x="540" y="${interpY + 116}" text-anchor="middle" font-family="'Noto Sans KR','Apple SD Gothic Neo',NanumSquare,sans-serif" font-size="34" fill="#F4F8FF" font-weight="300" letter-spacing="1">가짜가 되는 건 아니에요.</text>
    </g>
...
async function main() {
  console.log('=== 2026-09-16_wed 소개형 마이너 (Knight of Cups) ===')
