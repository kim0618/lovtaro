/**
 * 2026-04-21 화요일 shortform 커버 이미지
 * - scene01 베이스 유지
 * - 상단 문구를 카드 가운데(중앙)로 이동 - 카드 위에 오버레이
 * - 번호/하단 텍스트 제거 (커버 전용 간결 구성)
 *
 * 실행: node scripts/generate-cover-2026-04-21_tue.mjs
 */
...
async function generateCover() {
  ...
  const svg = `<svg xmlns="http://www.w3.org/2000/svg" width="${W}" height="${H}">
    ...
    <!-- 타이틀 가독성용 어두운 오버레이 (카드 중앙) -->
    <ellipse cx="540" cy="${cardY + 12}" rx="540" ry="130" fill="url(#titleBgGlow)"/>

    <!-- 상단 문구를 카드 가운데로 이동 -->
    <g filter="url(#softGlow)">
      <text x="540" y="${titleY1}" text-anchor="middle" font-family="'Noto Sans KR','Apple SD Gothic Neo',NanumSquare,sans-serif" font-size="58" fill="#F4F8FF" letter-spacing="3" font-weight="400">그 사람 마음 속에서</text>
      <text x="540" y="${titleY2}" text-anchor="middle" font-family="'Noto Sans KR','Apple SD Gothic Neo',NanumSquare,sans-serif" font-size="58" fill="#F4F8FF" letter-spacing="3" font-weight="400">나는 어떤 존재일까?</text>
    </g>
...
async function main() {
  console.log('=== 2026-04-21 shortform 커버 이미지 생성 ===')
  await generateCover()
  console.log('완료!')
}
