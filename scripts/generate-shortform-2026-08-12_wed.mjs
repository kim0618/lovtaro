...
const outputDir = resolve(rootDir, 'content-output/2026-08-12_wed/insta')
const W = 1080, H = 1920
...
async function scene01() {
  ...
  const hookLineTop = '마음은 있어 보이는데,'
  const accentBefore = '왜 자꾸 '
  const accentWord = '재는'
  const accentAfter = ' 느낌이 들까?'
  ...
  // Seven of Swords 원본이 야간 장면이라 기본값(brightness 0.82)이면 인물·검이 전부 묻힘 → 이 카드만 상향
  const cardBg = await sharp(cardRaw)
    .blur(4)
    .modulate({ saturation: 1.15, brightness: 1.45 })
    .toBuffer()
...
async function scene02() {
  ...
  const svg = `<svg xmlns="http://www.w3.org/2000/svg" width="${W}" height="${H}">
    ...
    <text x="540" y="${kwY}" text-anchor="middle" font-family="'Noto Sans KR','Apple SD Gothic Neo',NanumSquare,sans-serif" font-size="26" fill="rgba(232,212,139,0.72)" letter-spacing="4" font-weight="300">기만 · 회피 · 숨겨진 것</text>
...
async function scene03() {
  ...
  const svg = `<svg xmlns="http://www.w3.org/2000/svg" width="${W}" height="${H}">
    ...
    <g filter="url(#softGlow)">
      <text x="540" y="${headerY}" text-anchor="middle" font-family="'Noto Sans KR','Apple SD Gothic Neo',NanumSquare,sans-serif" font-size="40" fill="#F4F8FF" font-weight="300" letter-spacing="6">말을 아끼는 사람의 속마음</text>
    </g>
    ...
    <text x="540" y="${nameKrY}" text-anchor="middle" font-family="'Noto Sans KR','Apple SD Gothic Neo',NanumSquare,sans-serif" font-size="34" fill="#F4F8FF" font-weight="300" letter-spacing="4">소드의 7</text>
    <text x="540" y="${nameEnY}" text-anchor="middle" font-family="Georgia, serif" font-style="italic" font-size="20" fill="rgba(232,212,139,0.8)" letter-spacing="1">Seven of Swords</text>

    <text x="540" y="${kwY}" text-anchor="middle" font-family="'Noto Sans KR','Apple SD Gothic Neo',NanumSquare,sans-serif" font-size="22" fill="rgba(232,212,139,0.6)" letter-spacing="4" font-weight="300">기만 · 회피 · 숨겨진 것</text>

    <g filter="url(#softGlow)">
      <text x="540" y="${interpY}" text-anchor="middle" font-family="'Noto Sans KR','Apple SD Gothic Neo',NanumSquare,sans-serif" font-size="34" fill="#F4F8FF" font-weight="300" letter-spacing="1">마음이 없는 게 아니라,</text>
      <text x="540" y="${interpY + 58}" text-anchor="middle" font-family="'Noto Sans KR','Apple SD Gothic Neo',NanumSquare,sans-serif" font-size="34" fill="#F4F8FF" font-weight="300" letter-spacing="1">먼저 다칠까 봐 재고 있는 거예요.</text>
      <text x="540" y="${interpY + 116}" text-anchor="middle" font-family="'Noto Sans KR','Apple SD Gothic Neo',NanumSquare,sans-serif" font-size="34" fill="#F4F8FF" font-weight="300" letter-spacing="1">먼저 안심시켜 주면 달라질 수 있어요.</text>
    </g>
