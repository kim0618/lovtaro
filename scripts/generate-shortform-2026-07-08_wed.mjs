/**
 * 소개형 마이너 shortform 2026-07-08_wed (Three of Swords)
 * 훅: 괜찮아진 줄 알았는데, / 왜 그 이름에 아직 마음이 저릴까?
 */
...
const outputDir = resolve(rootDir, 'content-output/2026-07-08_wed/shortform')
const W = 1080, H = 1920

const PORTRAIT_OVERRIDE = {
  'three-of-swords': 'public/images/mcards/swords/Three of Swords.png',
}
...
async function scene01() {
  const cardSlug = 'three-of-swords'
  const hookLineTop = '괜찮아진 줄 알았는데,'
  const accentBefore = '왜 그 이름에 아직 '
  const accentWord = '마음이 저릴까'
  const accentAfter = '?'
...
async function scene02() {
  ...
  const cardRaw = await loadCard('three-of-swords', cardW, cardH)
  ...
  const svg = `<svg xmlns="http://www.w3.org/2000/svg" width="${W}" height="${H}">
    ...
    <g filter="url(#softGlow)">
      <text x="540" y="${nameKrY}" text-anchor="middle" font-family="'Noto Sans KR','Apple SD Gothic Neo',NanumSquare,sans-serif" font-size="58" fill="#F4F8FF" font-weight="300" letter-spacing="4">소드의 3</text>
      <text x="540" y="${nameEnY}" text-anchor="middle" font-family="Georgia, serif" font-style="italic" font-size="30" fill="rgba(232,212,139,0.88)" letter-spacing="1">Three of Swords</text>
    </g>

    <text x="540" y="${kwY}" text-anchor="middle" font-family="'Noto Sans KR','Apple SD Gothic Neo',NanumSquare,sans-serif" font-size="26" fill="rgba(232,212,139,0.72)" letter-spacing="4" font-weight="300">이별 · 상처 · 슬픔</text>
...
async function scene03() {
  ...
  const cardImg = await loadCard('three-of-swords', cardW, cardH)
  ...
  const svg = `<svg xmlns="http://www.w3.org/2000/svg" width="${W}" height="${H}">
    ...
    <g filter="url(#softGlow)">
      <text x="${W / 2}" y="${headerY}" text-anchor="middle" font-family="'Noto Sans KR','Apple SD Gothic Neo',NanumSquare,sans-serif" font-size="40" fill="#F4F8FF" font-weight="300" letter-spacing="6">아픔이 지나간 자리, 그 마음을 들여다보면</text>
    </g>
    ...
    <text x="${W / 2}" y="${nameKrY}" text-anchor="middle" font-family="'Noto Sans KR','Apple SD Gothic Neo',NanumSquare,sans-serif" font-size="34" fill="#F4F8FF" font-weight="300" letter-spacing="4">소드의 3</text>
    <text x="${W / 2}" y="${nameEnY}" text-anchor="middle" font-family="Georgia, serif" font-style="italic" font-size="20" fill="rgba(232,212,139,0.8)" letter-spacing="1">Three of Swords</text>

    <text x="${W / 2}" y="${kwY}" text-anchor="middle" font-family="'Noto Sans KR','Apple SD Gothic Neo',NanumSquare,sans-serif" font-size="22" fill="rgba(232,212,139,0.6)" letter-spacing="4" font-weight="300">이별 · 상처 · 슬픔</text>

    <g filter="url(#softGlow)">
      <text x="${W / 2}" y="${interpY}" text-anchor="middle" font-family="'Noto Sans KR','Apple SD Gothic Neo',NanumSquare,sans-serif" font-size="34" fill="#F4F8FF" font-weight="300" letter-spacing="1">지나간 일이라 여겼는데 문득 마음이 저리는 건,</text>
      <text x="${W / 2}" y="${interpY + 58}" text-anchor="middle" font-family="'Noto Sans KR','Apple SD Gothic Neo',NanumSquare,sans-serif" font-size="34" fill="#F4F8FF" font-weight="300" letter-spacing="1">그 감정을 아직 다 흘려보내지 못했다는 신호일 수 있어요.</text>
      <text x="${W / 2}" y="${interpY + 116}" text-anchor="middle" font-family="'Noto Sans KR','Apple SD Gothic Neo',NanumSquare,sans-serif" font-size="34" fill="#F4F8FF" font-weight="300" letter-spacing="1">밀어내기보다 천천히 바라볼 때, 조금씩 옅어지기 시작해요.</text>
    </g>
...
async function main() {
  console.log('=== 7/8 수 소개형 마이너 (Three of Swords) ===')
  mkdirSync(outputDir, { recursive: true })
  await scene01()
  await scene02()
  await scene03()
