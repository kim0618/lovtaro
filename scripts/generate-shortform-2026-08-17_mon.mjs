...
const outputDir = resolve(rootDir, 'content-output/2026-08-17_mon/insta')
const W = 1080, H = 1920
...
const PORTRAIT_OVERRIDE = {
  ...
  'king-of-wands': 'public/images/mcards/wands/King of Wands.png',
  'five-of-cups': 'public/images/mcards/cups/Five of Cups.png',
  // 신규 카드 사용 시 여기 추가
}
...
async function scene01() {
  ...
  const cardSlug = 'five-of-cups'
  const hookLineTop = '헤어진 건 한참 전인데,'
  const accentBefore = '마음만 아직 '
  const accentWord = '그날에'
  const accentAfter = ' 남아 있을 때'
  ...
  // Five of Cups 원본이 야간 강가 장면이라 기본값(0.82)이면 망토 인물·쏟아진 잔이 전부 묻힘 → 이 카드만 상향
  const cardBg = await sharp(cardRaw)
    .blur(4)
    .modulate({ saturation: 1.10, brightness: 1.75 })
    .toBuffer()
...
async function scene02() {
  ...
  const cardRaw = await loadCard('five-of-cups', cardW, cardH)
  ...
  const svg = `<svg xmlns="http://www.w3.org/2000/svg" width="${W}" height="${H}">
    ...
    <g filter="url(#softGlow)">
      <text x="540" y="${nameKrY}" text-anchor="middle" font-family="'Noto Sans KR','Apple SD Gothic Neo',NanumSquare,sans-serif" font-size="58" fill="#F4F8FF" font-weight="300" letter-spacing="4">컵의 5</text>
      <text x="540" y="${nameEnY}" text-anchor="middle" font-family="Georgia, serif" font-style="italic" font-size="30" fill="rgba(232,212,139,0.88)" letter-spacing="1">Five of Cups</text>
    </g>

    <text x="540" y="${kwY}" text-anchor="middle" font-family="'Noto Sans KR','Apple SD Gothic Neo',NanumSquare,sans-serif" font-size="26" fill="rgba(232,212,139,0.72)" letter-spacing="4" font-weight="300">상실 · 후회 · 슬픔</text>
...
async function scene03() {
  ...
  const cardImg = await loadCard('five-of-cups', cardW, cardH)
  ...
  const svg = `<svg xmlns="http://www.w3.org/2000/svg" width="${W}" height="${H}">
    ...
    <g filter="url(#softGlow)">
      <text x="540" y="${headerY}" text-anchor="middle" font-family="'Noto Sans KR','Apple SD Gothic Neo',NanumSquare,sans-serif" font-size="40" fill="#F4F8FF" font-weight="300" letter-spacing="6">지나간 자리에 머무는 마음</text>
    </g>
    ...
    <text x="540" y="${nameKrY}" text-anchor="middle" font-family="'Noto Sans KR','Apple SD Gothic Neo',NanumSquare,sans-serif" font-size="34" fill="#F4F8FF" font-weight="300" letter-spacing="4">컵의 5</text>
    <text x="540" y="${nameEnY}" text-anchor="middle" font-family="Georgia, serif" font-style="italic" font-size="20" fill="rgba(232,212,139,0.8)" letter-spacing="1">Five of Cups</text>

    <text x="540" y="${kwY}" text-anchor="middle" font-family="'Noto Sans KR','Apple SD Gothic Neo',NanumSquare,sans-serif" font-size="22" fill="rgba(232,212,139,0.6)" letter-spacing="4" font-weight="300">상실 · 후회 · 슬픔</text>

    <g filter="url(#softGlow)">
      <text x="540" y="${interpY}" text-anchor="middle" font-family="'Noto Sans KR','Apple SD Gothic Neo',NanumSquare,sans-serif" font-size="34" fill="#F4F8FF" font-weight="300" letter-spacing="1">쏟아진 잔만 오래 바라보는 시기예요.</text>
      <text x="540" y="${interpY + 58}" text-anchor="middle" font-family="'Noto Sans KR','Apple SD Gothic Neo',NanumSquare,sans-serif" font-size="34" fill="#F4F8FF" font-weight="300" letter-spacing="1">등 뒤에는 아직 서 있는 잔이 있어요.</text>
      <text x="540" y="${interpY + 116}" text-anchor="middle" font-family="'Noto Sans KR','Apple SD Gothic Neo',NanumSquare,sans-serif" font-size="34" fill="#F4F8FF" font-weight="300" letter-spacing="1">문제는 마음이 아니라 시선의 방향이에요.</text>
    </g>
...
async function main() {
  console.log('=== 2026-07-29_wed 소개형 마이너 shortform ===')
