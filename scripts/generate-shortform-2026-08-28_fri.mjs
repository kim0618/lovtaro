...
const outputDir = resolve(rootDir, 'content-output/2026-08-28_fri/insta')
const W = 1080, H = 1920
...
const PORTRAIT_OVERRIDE = {
  'knight-of-swords': 'public/images/mcards/swords/Knight of Swords.png',
...
async function scene01() {
  ...
  const cardSlug = 'knight-of-swords'
  const hookLineTop = '만난 지 얼마 안 됐는데,'
  const accentBefore = '벌써 그 '
  const accentWord = '확신'
  const accentAfter = '이 부담스러울 때'
  ...
  const cardBg = await sharp(cardRaw)
    .blur(4)
    .modulate({ saturation: 1.14, brightness: 1.60 })
    .toBuffer()
...
async function scene02() {
  ...
  const cardRaw = await loadCard('knight-of-swords', cardW, cardH)
  ...
  const svg = `<svg xmlns="http://www.w3.org/2000/svg" width="${W}" height="${H}">
    ...
    <g filter="url(#softGlow)">
      <text x="540" y="${nameKrY}" text-anchor="middle" font-family="'Noto Sans KR','Apple SD Gothic Neo',NanumSquare,sans-serif" font-size="58" fill="#F4F8FF" font-weight="300" letter-spacing="4">소드의 나이트</text>
      <text x="540" y="${nameEnY}" text-anchor="middle" font-family="Georgia, serif" font-style="italic" font-size="30" fill="rgba(232,212,139,0.88)" letter-spacing="1">Knight of Swords</text>
    </g>

    <text x="540" y="${kwY}" text-anchor="middle" font-family="'Noto Sans KR','Apple SD Gothic Neo',NanumSquare,sans-serif" font-size="26" fill="rgba(232,212,139,0.72)" letter-spacing="4" font-weight="300">돌진 · 급진적 행동 · 갈등</text>
...
async function scene03() {
  ...
  const cardImg = await loadCard('knight-of-swords', cardW, cardH)
  ...
  const svg = `<svg xmlns="http://www.w3.org/2000/svg" width="${W}" height="${H}">
    ...
    <g filter="url(#softGlow)">
      <text x="540" y="${headerY}" text-anchor="middle" font-family="'Noto Sans KR','Apple SD Gothic Neo',NanumSquare,sans-serif" font-size="40" fill="#F4F8FF" font-weight="300" letter-spacing="6">너무 빨리 다가오는 마음</text>
    </g>
    ...
    <text x="540" y="${nameKrY}" text-anchor="middle" font-family="'Noto Sans KR','Apple SD Gothic Neo',NanumSquare,sans-serif" font-size="34" fill="#F4F8FF" font-weight="300" letter-spacing="4">소드의 나이트</text>
    <text x="540" y="${nameEnY}" text-anchor="middle" font-family="Georgia, serif" font-style="italic" font-size="20" fill="rgba(232,212,139,0.8)" letter-spacing="1">Knight of Swords</text>

    <text x="540" y="${kwY}" text-anchor="middle" font-family="'Noto Sans KR','Apple SD Gothic Neo',NanumSquare,sans-serif" font-size="22" fill="rgba(232,212,139,0.6)" letter-spacing="4" font-weight="300">돌진 · 급진적 행동 · 갈등</text>

    <g filter="url(#softGlow)">
      <text x="540" y="${interpY}" text-anchor="middle" font-family="'Noto Sans KR','Apple SD Gothic Neo',NanumSquare,sans-serif" font-size="34" fill="#F4F8FF" font-weight="300" letter-spacing="1">빠른 확신이 곧 깊이는 아니에요.</text>
      <text x="540" y="${interpY + 58}" text-anchor="middle" font-family="'Noto Sans KR','Apple SD Gothic Neo',NanumSquare,sans-serif" font-size="34" fill="#F4F8FF" font-weight="300" letter-spacing="1">지금은 속도를 맞추기보다,</text>
      <text x="540" y="${interpY + 116}" text-anchor="middle" font-family="'Noto Sans KR','Apple SD Gothic Neo',NanumSquare,sans-serif" font-size="34" fill="#F4F8FF" font-weight="300" letter-spacing="1">내 속도를 지켜도 괜찮아요.</text>
    </g>
...
async function main() {
  console.log('=== 2026-08-28 금요일 소개형 마이너 (Knight of Swords) ===')
