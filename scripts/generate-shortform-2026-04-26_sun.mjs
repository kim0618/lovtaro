...
const CARDS = [
  ...
  { num: '②', slug: 'knight-of-cups', imageSrc: 'public/images/cards/knight-of-cups.webp',
    nameKr: '컵 기사',     nameEn: 'Knight of Cups', keywords: '다가옴 · 감정 · 접근',
    hook: ['수~목 사이,', '감정의 거리감이 줄어들 수 있어요'] },
  { num: '③', slug: 'ace-of-wands',   imageSrc: 'public/images/mcards/wands/Ace of Wands.png',
    nameKr: '완드 에이스', nameEn: 'Ace of Wands',   keywords: '점화 · 움직임 · 시작',
...
async function scene01() {
  ...
  const svg = `<svg xmlns="http://www.w3.org/2000/svg" width="${W}" height="${H}">
    ...
    <g filter="url(#softGlow)">
      ...
      <text x="540" y="505" text-anchor="middle" font-family="${KO_STACK}" font-size="58" fill="#F4F8FF" letter-spacing="2" font-weight="300">월요일 전에 미리 확인하세요</text>
    </g>
...
async function scene02() {
  ...
  const svg = `<svg xmlns="http://www.w3.org/2000/svg" width="${W}" height="${H}">
    ...
    <g filter="url(#midNumGlow)">
      <text x="540" y="${numberY + 110}" text-anchor="middle" font-family="${KO_STACK}" font-size="32" fill="rgba(244,248,255,0.95)" letter-spacing="4" font-weight="400">멈추고 선택해보세요</text>
    </g>
...
async function scene07() {
  ...
  const svg = `<svg xmlns="http://www.w3.org/2000/svg" width="${W}" height="${H}">
    ...
    <g filter="url(#softGlow)">
      <text x="540" y="260" text-anchor="middle" font-family="${KO_STACK}" font-size="56" fill="#F4F8FF" letter-spacing="2" font-weight="300">저장해두고</text>
      <text x="540" y="340" text-anchor="middle" font-family="${KO_STACK}" font-size="56" fill="#F4F8FF" letter-spacing="2" font-weight="300">다음 주에 다시 확인해보세요</text>
    </g>
    ...
    <g filter="url(#softGlow)">
      <text x="540" y="1260" text-anchor="middle" font-family="${KO_STACK}" font-size="36" fill="#F4F8FF" letter-spacing="2" font-weight="300">고른 번호를 댓글에 남겨주세요</text>
      <text x="540" y="1325" text-anchor="middle" font-family="${KO_STACK}" font-size="36" fill="#F4F8FF" letter-spacing="2" font-weight="300">답글로 더 깊은 해석을 남겨드릴게요</text>
    </g>

    <g opacity="0.85" filter="url(#softGlow)">
      <circle cx="490" cy="1455" r="6" fill="#e8d48b"/>
      <circle cx="540" cy="1455" r="6" fill="#e8d48b"/>
      <circle cx="590" cy="1455" r="6" fill="#e8d48b"/>
    </g>
...
async function main() {
  console.log('=== 2026-04-26 Sunday Tarot Preview (7컷) ===')
  mkdirSync(outputDir, { recursive: true })
  await scene01()
  await scene02()
  await scene03()
  await revealScene(CARDS[0], 101, 'scene04.png')
  await revealScene(CARDS[1], 202, 'scene05.png')
  await revealScene(CARDS[2], 303, 'scene06.png')
  await scene07()
  console.log('완료!')
}
