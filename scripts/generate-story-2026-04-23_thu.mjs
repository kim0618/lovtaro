/**
 * 2026-04-23 목요일 story 이미지 생성 (마이너 아르카나)
 * - story01: Knight of Wands + "뜨겁게 다가오는 에너지"
 *
 * 실행: node scripts/generate-story-2026-04-23_thu.mjs
 */
...
const outputDir = resolve(rootDir, 'content-output/2026-04-23_thu/story')
const W = 1080, H = 1920
...
async function main() {
  console.log('=== 2026-04-23 story 이미지 생성 ===')
  ...
  const img = await loadCard('wands', 'Knight of Wands', cW, cH)
  if (!img) { console.error('❌ Knight of Wands.png 없음'); process.exit(1) }
  ...
  const hookLines = ['뜨겁게 다가오는 에너지', '예상치 못한 움직임이 있을 수 있어요']
  const keywords = ['열정', '추진력', '빠른 흐름']
