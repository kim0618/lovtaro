/**
 * 2026-04-24 금요일 story 이미지 생성 (마이너 아르카나)
 * - story01: Queen of Wands + "자신감 있는 매력이 빛나는 하루"
 *
 * 실행: node scripts/generate-story-2026-04-24_fri.mjs
 */
...
const outputDir = resolve(rootDir, 'content-output/2026-04-24_fri/story')
const W = 1080, H = 1920
...
async function main() {
  console.log('=== 2026-04-24 story 이미지 생성 ===')
  ...
  const img = await loadCard('wands', 'Queen of Wands', cW, cH)
  if (!img) { console.error('❌ Queen of Wands.png 없음'); process.exit(1) }
  ...
  const hookLines = ['자신감 있는 매력이 빛나는 하루', '오늘의 나를 그대로 드러내도 괜찮아요']
  const keywords = ['당당함', '매력', '자기 확신']
  ...
  <text x="540" y="${engNameY}" font-family="Georgia, serif" font-size="22" font-style="italic" fill="rgba(200,180,140,0.65)" text-anchor="middle">Queen of Wands</text>
  <g filter="url(#tg)">
    ${hookSvg}
  </g>
