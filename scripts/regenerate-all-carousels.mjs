/**
 * 전체 캐러셀 일괄 재생성 (v3 스타일)
 * - 표지: 카드 풀사이즈 + 하단 페이드 텍스트
 * - 본문(edu): 카드 풀배경 반투명 + 텍스트 중앙
 * - 요일 카드(weekly): 카드 상단 + 하단 텍스트 (기존 유지하되 스타일 통일)
 * - 테스트: 카드 풀배경 + 텍스트
 * - CTA: 카드 반투명 + 저장/공유
 *
 * 실행: node scripts/regenerate-all-carousels.mjs
 */
...
async function coverSlide(cardSlug, title, subtitle) {
  ...
  if (masked) {
    ...
    <text x="540" y="1100" font-family="sans-serif" font-size="52" font-weight="600" fill="#FFFFFF" text-anchor="middle">${esc(title)}</text>
    <text x="540" y="1155" font-family="sans-serif" font-size="28" fill="rgba(210,200,250,0.7)" text-anchor="middle">${esc(subtitle)}</text>
...
// 요일 카드 슬라이드 (주간 운세용) - 카드 크게 + 하단 텍스트
async function dayCardSlide(card) {
  ...
  const nameY = cardTop + cH + 50
  const lineStartY = nameY + 50

  const lineSvg = lines.map((l, i) =>
    `<text x="540" y="${lineStartY + i * 42}" font-family="sans-serif" font-size="28" font-weight="300" fill="rgba(230,225,250,0.85)" text-anchor="middle">${esc(l)}</text>`
  ).join('\n  ')

  const svg = bgSvg(`
  <text x="540" y="55" font-family="sans-serif" font-size="24" fill="rgba(180,170,230,0.7)" text-anchor="middle" letter-spacing="6">${esc(card.day)}요일</text>
  ...
  <text x="540" y="${nameY}" font-family="sans-serif" font-size="38" font-weight="400" fill="#FFFFFF" text-anchor="middle">${esc(card.name)}</text>
  ${lineSvg}
  `)
...
async function testTypeSlide(item) {
  ...
  const descSvg = lines.map((l, i) =>
    `<text x="540" y="${730 + i * 44}" font-family="sans-serif" font-size="28" font-weight="300" fill="rgba(240,238,255,0.95)" text-anchor="middle">${esc(l)}</text>`
  ).join('\n  ')
  ...
  const overlay = `<svg xmlns="http://www.w3.org/2000/svg" width="${W}" height="${H}">
  <rect width="${W}" height="${H}" fill="rgba(8,6,26,0.45)"/>
  <text x="540" y="200" font-family="sans-serif" font-size="64" font-weight="600" fill="rgba(180,170,230,0.7)" text-anchor="middle">${esc(item.type)}</text>
  <text x="540" y="260" font-family="sans-serif" font-size="30" fill="rgba(240,238,255,0.85)" text-anchor="middle">${esc(item.label)}</text>
  <line x1="350" y1="300" x2="730" y2="300" stroke="rgba(180,170,230,0.2)" stroke-width="1"/>
  <text x="540" y="660" font-family="sans-serif" font-size="36" font-weight="500" fill="#FFFFFF" text-anchor="middle">${esc(item.cardName)}</text>
  ${descSvg}
  </svg>`
  base = await sharp(base).composite([{ input: Buffer.from(overlay), left: 0, top: 0 }]).png().toBuffer()
  return base
...
async function ctaSlide(cardSlug, saveLines, shareLines) {
  ...
  const saveIcon = `<g transform="translate(430, 900)"><path d="M14 0 L72 0 Q86 0 86 14 L86 105 L43 78 L0 105 L0 14 Q0 0 14 0 Z" fill="none" stroke="rgba(200,190,255,0.55)" stroke-width="2.5"/></g>`
  const shareIcon = `<g transform="translate(570, 900)"><path d="M58 0 L86 30 L58 60" fill="none" stroke="rgba(200,190,255,0.55)" stroke-width="2.5" stroke-linecap="round" stroke-linejoin="round"/><path d="M86 30 L14 30 L14 86" fill="none" stroke="rgba(200,190,255,0.55)" stroke-width="2.5" stroke-linecap="round" stroke-linejoin="round"/></g>`
  ...
  ${saveIcon} ${shareIcon}
  <text x="540" y="1060" font-family="sans-serif" font-size="36" font-weight="500" fill="#FFFFFF" text-anchor="middle">${esc(sLine1)}</text>
  <text x="540" y="1105" font-family="sans-serif" font-size="24" fill="rgba(210,200,250,0.65)" text-anchor="middle">${esc(sLine2)}</text>
  <line x1="400" y1="1140" x2="680" y2="1140" stroke="rgba(180,170,230,0.12)" stroke-width="1"/>
  <text x="540" y="1180" font-family="sans-serif" font-size="22" fill="rgba(200,190,240,0.5)" text-anchor="middle">${esc(shLine1)}</text>
  ${shLine2 ? `<text x="540" y="1212" font-family="sans-serif" font-size="22" fill="rgba(200,190,240,0.5)" text-anchor="middle">${esc(shLine2)}</text>` : ''}
  `)
...
async function main() {
  console.log(`전체 ${ALL_CAROUSELS.length}개 캐러셀 재생성 시작\n`)

  for (const c of ALL_CAROUSELS) {
    const dir = resolve(outputDir, c.folder, 'carousel')
    ensureDir(dir)
    let slideNum = 1
