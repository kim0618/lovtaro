/**
 * 소개형 마이너 shortform 2026-05-27_wed (Four of Cups)
 * 훅: 옆에 있어도, / 왜 그 사람은 더 멀어 보일까?
 */
...
const outputDir = resolve(rootDir, 'content-output/2026-05-27_wed/shortform')
const W = 1080, H = 1920
...
async function scene01() {
  ...
  const hookLineTop = '옆에 있어도,'
  const accentBefore = '왜 그 사람은 더 '
  const accentWord = '멀어'
  const accentAfter = ' 보일까?'
...
async function scene03() {
  ...
  const svg = `<svg xmlns="http://www.w3.org/2000/svg" width="${W}" height="${H}">
    ...
    <g filter="url(#softGlow)">
      <text x="${W / 2}" y="${headerY}" text-anchor="middle" font-family="'Noto Sans KR','Apple SD Gothic Neo',NanumSquare,sans-serif" font-size="40" fill="#F4F8FF" font-weight="300" letter-spacing="6">내밀어진 손을 못 보는 시간</text>
    </g>
    ...
    <g filter="url(#softGlow)">
      <text x="${W / 2}" y="${interpY}" text-anchor="middle" font-family="'Noto Sans KR','Apple SD Gothic Neo',NanumSquare,sans-serif" font-size="34" fill="#F4F8FF" font-weight="300" letter-spacing="1">옆에 있는 사람이 어느 순간 멀게 느껴져요.</text>
      <text x="${W / 2}" y="${interpY + 58}" text-anchor="middle" font-family="'Noto Sans KR','Apple SD Gothic Neo',NanumSquare,sans-serif" font-size="34" fill="#F4F8FF" font-weight="300" letter-spacing="1">익숙해진 시야가 진짜 결을 가리고 있어요.</text>
      <text x="${W / 2}" y="${interpY + 116}" text-anchor="middle" font-family="'Noto Sans KR','Apple SD Gothic Neo',NanumSquare,sans-serif" font-size="34" fill="#F4F8FF" font-weight="300" letter-spacing="1">내밀어진 손을 다시 알아차릴 때예요.</text>
    </g>
...
async function main() {
  console.log('=== 2026-05-27 소개형 마이너 shortform (Four of Cups) ===')
  mkdirSync(outputDir, { recursive: true })
  await scene01()
  await scene02()
  await scene03()
