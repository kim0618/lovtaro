/**
 * 2026-04-20 월요일 shortform 이미지 생성 (소개형 메이저 NEW 디자인)
 * - scene01_new: 1.png 배경 + 훅 + @lovtarot_
 * - scene02_new: lovers.png 풀블리드 cover + @lovtarot_
 * - scene03_new: 3.png 배경 + 작은 프레임 카드(Lovers) + 해석 3줄 + CTA
 *
...
async function scene01() {
  ...
  const overlaySvg = `<svg xmlns="http://www.w3.org/2000/svg" width="${W}" height="${H}">
    ...
    <g filter="url(#textShadow)">
      <text x="540" y="360" text-anchor="middle" class="hook" fill="#FFFFFF">이 사람과 나,</text>
      <text x="540" y="442" text-anchor="middle" class="hook" fill="#FFFFFF">정말 <tspan class="hook-accent">운명</tspan>일까?</text>
    </g>
...
async function scene02() {
  ...
  const cardRaw = await sharp(resolve(cardsDir, 'lovers.png'))
  ...
  const svg = `<svg xmlns="http://www.w3.org/2000/svg" width="${W}" height="${H}">
    ...
    <g filter="url(#softGlow)">
      <text x="540" y="${nameKrY}" text-anchor="middle" font-family="'Noto Sans KR', sans-serif" font-size="52" fill="#F4F8FF" font-weight="300" letter-spacing="3">연인</text>
      <text x="540" y="${nameEnY}" text-anchor="middle" font-family="Georgia, serif" font-style="italic" font-size="30" fill="rgba(232,212,139,0.88)" letter-spacing="1">The Lovers</text>
    </g>

    <text x="540" y="${kwY}" text-anchor="middle" font-family="'Noto Sans KR', sans-serif" font-size="26" fill="rgba(232,212,139,0.72)" letter-spacing="4" font-weight="300">운명적 끌림 · 진심의 선택</text>
...
async function scene03() {
  ...
  const overlaySvg = `<svg xmlns="http://www.w3.org/2000/svg" width="${W}" height="${H}">
    ...
    <g filter="url(#textShadow)">
      <text x="540" y="220" text-anchor="middle" class="header" fill="#FFFFFF">운명의 <tspan class="hook-accent">끌림</tspan></text>
    </g>
    ...
    <g filter="url(#textShadow)">
      <text x="540" y="${nameKrY}" text-anchor="middle" class="card-name-kr-sm" fill="#FFFFFF">연인</text>
      <text x="540" y="${nameEnY}" text-anchor="middle" class="card-name-en-sm" fill="#E8C877">The Lovers</text>
    </g>

    <g filter="url(#textShadow)">
      <text x="540" y="${kwY}" text-anchor="middle" class="keywords-sm" fill="#E8C877">운명적 끌림 · 진심의 선택</text>
    </g>

    <g filter="url(#textShadow)">
      <text x="540" y="${interpY}"        text-anchor="middle" class="interp" fill="#FFFFFF">지금 느끼는 끌림은 진짜예요.</text>
      <text x="540" y="${interpY + 70}"  text-anchor="middle" class="interp" fill="#FFFFFF">당신의 선택이 이 관계를</text>
      <text x="540" y="${interpY + 140}" text-anchor="middle" class="interp" fill="#FFFFFF"><tspan class="hook-accent">운명</tspan>으로 완성할 거예요.</text>
    </g>
...
async function main() {
  console.log('=== 2026-04-20 월요일 소개형 메이저 NEW 디자인 ===')
