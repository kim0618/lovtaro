/**
 * 2026-04-25 토요일 story 이미지 생성 (마이너 아르카나)
 * - story01: Ten of Pentacles + "오래 이어지는 안정된 마음"
 *
 * 실행: node scripts/generate-story-2026-04-25_sat.mjs
 */
...
const outputDir = resolve(rootDir, 'content-output/2026-04-25_sat/story')
const W = 1080, H = 1920
...
async function main() {
  console.log('=== 2026-04-25 story 이미지 생성 ===')
  ...
  const img = await loadCard('pentacles', 'Ten of Pentacles', cW, cH)
  if (!img) { console.error('❌ Ten of Pentacles.png 없음'); process.exit(1) }
  ...
  const hookLines = ['오래 이어지는 안정된 마음', '꾸준한 관계가 단단해지는 주말']
  const keywords = ['안정', '지속', '뿌리 깊은 감정']
  ...
  <text x="540" y="${engNameY}" font-family="Georgia, serif" font-size="22" font-style="italic" fill="rgba(200,180,140,0.65)" text-anchor="middle">Ten of Pentacles</text>
  <g filter="url(#tg)">
    ${hookSvg}
  </g>
