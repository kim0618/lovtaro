/**
 * 소개형 마이너 shortform 2026-06-05_fri (Seven of Cups)
 * 훅: 다 말해주는 것 같은데, / 왜 늘 중요한 건 빠져 있을까?
 */
...
const outputDir = resolve(rootDir, 'content-output/2026-06-19_fri/shortform')
const W = 1080, H = 1920

const PORTRAIT_OVERRIDE = {
  'two-of-pentacles': 'public/images/mcards/pentacles/Two of Pentacles.png',
}
...
async function scene01() {
  const cardSlug = 'two-of-pentacles'
  const hookLineTop = '다 맞춰주는 사람인데,'
  const accentBefore = '왜 나한테는 자꾸 '
  const accentWord = '늦을까'
  const accentAfter = '?'
...
async function scene02() {
  ...
  const cardRaw = await loadCard('two-of-pentacles', cardW, cardH)
  ...
  const svg = `<svg xmlns="http://www.w3.org/2000/svg" width="${W}" height="${H}">
    ...
    <g filter="url(#softGlow)">
      <text x="540" y="${nameKrY}" text-anchor="middle" font-family="'Noto Sans KR','Apple SD Gothic Neo',NanumSquare,sans-serif" font-size="58" fill="#F4F8FF" font-weight="300" letter-spacing="4">펜타클의 2</text>
      <text x="540" y="${nameEnY}" text-anchor="middle" font-family="Georgia, serif" font-style="italic" font-size="30" fill="rgba(232,212,139,0.88)" letter-spacing="1">Two of Pentacles</text>
    </g>

    <text x="540" y="${kwY}" text-anchor="middle" font-family="'Noto Sans KR','Apple SD Gothic Neo',NanumSquare,sans-serif" font-size="26" fill="rgba(232,212,139,0.72)" letter-spacing="4" font-weight="300">균형 · 바쁨 · 우선순위</text>
...
async function scene03() {
  ...
  const cardImg = await loadCard('two-of-pentacles', cardW, cardH)
  ...
  const svg = `<svg xmlns="http://www.w3.org/2000/svg" width="${W}" height="${H}">
    ...
    <g filter="url(#softGlow)">
      <text x="${W / 2}" y="${headerY}" text-anchor="middle" font-family="'Noto Sans KR','Apple SD Gothic Neo',NanumSquare,sans-serif" font-size="40" fill="#F4F8FF" font-weight="300" letter-spacing="6">여러 가지를 동시에 감당하는 사람</text>
    </g>
    ...
    <text x="${W / 2}" y="${nameKrY}" text-anchor="middle" font-family="'Noto Sans KR','Apple SD Gothic Neo',NanumSquare,sans-serif" font-size="34" fill="#F4F8FF" font-weight="300" letter-spacing="4">펜타클의 2</text>
    <text x="${W / 2}" y="${nameEnY}" text-anchor="middle" font-family="Georgia, serif" font-style="italic" font-size="20" fill="rgba(232,212,139,0.8)" letter-spacing="1">Two of Pentacles</text>

    <text x="${W / 2}" y="${kwY}" text-anchor="middle" font-family="'Noto Sans KR','Apple SD Gothic Neo',NanumSquare,sans-serif" font-size="22" fill="rgba(232,212,139,0.6)" letter-spacing="4" font-weight="300">균형 · 바쁨 · 우선순위</text>

    <g filter="url(#softGlow)">
      <text x="${W / 2}" y="${interpY}" text-anchor="middle" font-family="'Noto Sans KR','Apple SD Gothic Neo',NanumSquare,sans-serif" font-size="34" fill="#F4F8FF" font-weight="300" letter-spacing="1">마음이 줄어든 게 아니라 감당할 게 많은 거예요.</text>
      <text x="${W / 2}" y="${interpY + 58}" text-anchor="middle" font-family="'Noto Sans KR','Apple SD Gothic Neo',NanumSquare,sans-serif" font-size="34" fill="#F4F8FF" font-weight="300" letter-spacing="1">그래서 연락도 약속도 예전보다 느려질 수 있어요.</text>
      <text x="${W / 2}" y="${interpY + 116}" text-anchor="middle" font-family="'Noto Sans KR','Apple SD Gothic Neo',NanumSquare,sans-serif" font-size="34" fill="#F4F8FF" font-weight="300" letter-spacing="1">재촉보다 조금 느긋하게 지켜보는 게 맞는 시기예요.</text>
    </g>
    ...
    <text x="${W / 2}" y="${ctaY + 54}" text-anchor="middle" font-family="sans-serif" font-size="26" fill="rgba(232,212,139,0.78)" letter-spacing="2">무료 타로 리딩 lovtaro.kr</text>

    <text x="${W / 2}" y="1870" text-anchor="middle" font-family="sans-serif" font-size="22" fill="rgba(232,212,139,0.4)" letter-spacing="4">@lovtarot_</text>
  </svg>`
...
async function main() {
  console.log('=== 2026-06-10 소개형 마이너 shortform (Two of Pentacles) ===')
  mkdirSync(outputDir, { recursive: true })
  await scene01()
  await scene02()
  await scene03()
