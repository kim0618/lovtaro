...
const outputDir = resolve(rootDir, 'content-output/2026-06-26_fri/shortform')
const W = 1080, H = 1920

const PORTRAIT_OVERRIDE = {
  'knight-of-pentacles': 'public/images/mcards/pentacles/Knight of Pentacles.png',
}
...
async function scene01() {
  const cardSlug = 'knight-of-pentacles'
  const hookLineTop = '무심한 듯 툭툭 던지는데,'
  const accentBefore = '왜 자꾸 '
  const accentWord = '마음에 남을까'
  const accentAfter = '?'
...
async function scene02() {
  ...
  const cardRaw = await loadCard('knight-of-pentacles', cardW, cardH)
  ...
  const svg = `<svg xmlns="http://www.w3.org/2000/svg" width="${W}" height="${H}">
    ...
    <g filter="url(#softGlow)">
      <text x="540" y="${nameKrY}" text-anchor="middle" font-family="'Noto Sans KR','Apple SD Gothic Neo',NanumSquare,sans-serif" font-size="58" fill="#F4F8FF" font-weight="300" letter-spacing="4">펜타클의 나이트</text>
      <text x="540" y="${nameEnY}" text-anchor="middle" font-family="Georgia, serif" font-style="italic" font-size="30" fill="rgba(232,212,139,0.88)" letter-spacing="1">Knight of Pentacles</text>
    </g>

    <text x="540" y="${kwY}" text-anchor="middle" font-family="'Noto Sans KR','Apple SD Gothic Neo',NanumSquare,sans-serif" font-size="26" fill="rgba(232,212,139,0.72)" letter-spacing="4" font-weight="300">성실 · 꾸준함 · 진심</text>
...
async function scene03() {
  ...
  const cardImg = await loadCard('knight-of-pentacles', cardW, cardH)
  ...
  const svg = `<svg xmlns="http://www.w3.org/2000/svg" width="${W}" height="${H}">
    ...
    <g filter="url(#softGlow)">
      <text x="${W / 2}" y="${headerY}" text-anchor="middle" font-family="'Noto Sans KR','Apple SD Gothic Neo',NanumSquare,sans-serif" font-size="40" fill="#F4F8FF" font-weight="300" letter-spacing="6">표현은 무뚝뚝해도 꾸준한 사람</text>
    </g>
    ...
    <text x="${W / 2}" y="${nameKrY}" text-anchor="middle" font-family="'Noto Sans KR','Apple SD Gothic Neo',NanumSquare,sans-serif" font-size="34" fill="#F4F8FF" font-weight="300" letter-spacing="4">펜타클의 나이트</text>
    <text x="${W / 2}" y="${nameEnY}" text-anchor="middle" font-family="Georgia, serif" font-style="italic" font-size="20" fill="rgba(232,212,139,0.8)" letter-spacing="1">Knight of Pentacles</text>

    <text x="${W / 2}" y="${kwY}" text-anchor="middle" font-family="'Noto Sans KR','Apple SD Gothic Neo',NanumSquare,sans-serif" font-size="22" fill="rgba(232,212,139,0.6)" letter-spacing="4" font-weight="300">성실 · 꾸준함 · 진심</text>

    <g filter="url(#softGlow)">
      <text x="${W / 2}" y="${interpY}" text-anchor="middle" font-family="'Noto Sans KR','Apple SD Gothic Neo',NanumSquare,sans-serif" font-size="34" fill="#F4F8FF" font-weight="300" letter-spacing="1">화려한 표현은 잘 하지 않는 사람이에요.</text>
      <text x="${W / 2}" y="${interpY + 58}" text-anchor="middle" font-family="'Noto Sans KR','Apple SD Gothic Neo',NanumSquare,sans-serif" font-size="34" fill="#F4F8FF" font-weight="300" letter-spacing="1">대신 약속한 건 끝까지 지키고 곁을 지켜요.</text>
      <text x="${W / 2}" y="${interpY + 116}" text-anchor="middle" font-family="'Noto Sans KR','Apple SD Gothic Neo',NanumSquare,sans-serif" font-size="34" fill="#F4F8FF" font-weight="300" letter-spacing="1">그 꾸준함이 말보다 더 큰 진심이에요.</text>
    </g>
...
async function main() {
  console.log('=== 6/26 금 소개형 마이너 (Knight of Pentacles) ===')
  mkdirSync(outputDir, { recursive: true })
  await scene01()
  await scene02()
  await scene03()
