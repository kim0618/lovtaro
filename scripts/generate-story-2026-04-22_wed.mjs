/**
 * 2026-04-22 수요일 story 이미지 생성 (마이너 아르카나)
 * - story01: Six of Pentacles + "주고받는 균형이 맞아가는 하루"
 *
 * 실행: node scripts/generate-story-2026-04-22_wed.mjs
 */
...
const outputDir = resolve(rootDir, 'content-output/2026-04-22_wed/story')
const W = 1080, H = 1920
...
async function main() {
  console.log('=== 2026-04-22 story 이미지 생성 ===')
  ...
  const img = await loadCard('pentacles', 'Six of Pentacles', cW, cH)
  if (!img) { console.error('❌ Six of Pentacles.png 없음'); process.exit(1) }
  ...
  const hookLines = ['주고받는 균형이 맞아가는 하루', '서로의 마음 속도가 비슷해져요']
  const keywords = ['균형', '주고받음', '관계의 흐름']
  ...
  <text x="540" y="${engNameY}" font-family="Georgia, serif" font-size="22" font-style="italic" fill="rgba(200,180,140,0.65)" text-anchor="middle">Six of Pentacles</text>
  <g filter="url(#tg)">
    ${hookSvg}
  </g>
