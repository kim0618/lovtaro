...
const outputDir = resolve(rootDir, 'content-output/2026-08-21_fri/insta')
const W = 1080, H = 1920
...
async function scene01() {
  ...
  const hookLineTop = '마음 다 준 것처럼 굴다가도,'
  const accentBefore = ''
  const accentWord = '확답'
  const accentAfter = ' 앞에선 멈칫하는 사람'
  ...
  // Two of Swords 원본 명도에 맞춰 조정 (기본값 0.82 기준)
  const cardBg = await sharp(cardRaw)
    .blur(4)
    .modulate({ saturation: 1.12, brightness: 1.18 })
    .toBuffer()
...
async function scene03() {
  ...
  const svg = `<svg xmlns="http://www.w3.org/2000/svg" width="${W}" height="${H}">
    ...
    <g filter="url(#softGlow)">
      <text x="540" y="${headerY}" text-anchor="middle" font-family="'Noto Sans KR','Apple SD Gothic Neo',NanumSquare,sans-serif" font-size="40" fill="#F4F8FF" font-weight="300" letter-spacing="6">확답을 미루는 사람의 속마음</text>
    </g>
    ...
    <text x="540" y="${nameKrY}" text-anchor="middle" font-family="'Noto Sans KR','Apple SD Gothic Neo',NanumSquare,sans-serif" font-size="34" fill="#F4F8FF" font-weight="300" letter-spacing="4">소드의 2</text>
    <text x="540" y="${nameEnY}" text-anchor="middle" font-family="Georgia, serif" font-style="italic" font-size="20" fill="rgba(232,212,139,0.8)" letter-spacing="1">Two of Swords</text>

    <text x="540" y="${kwY}" text-anchor="middle" font-family="'Noto Sans KR','Apple SD Gothic Neo',NanumSquare,sans-serif" font-size="22" fill="rgba(232,212,139,0.6)" letter-spacing="4" font-weight="300">결정 회피 · 균형 · 긴장</text>

    <g filter="url(#softGlow)">
      <text x="540" y="${interpY}" text-anchor="middle" font-family="'Noto Sans KR','Apple SD Gothic Neo',NanumSquare,sans-serif" font-size="34" fill="#F4F8FF" font-weight="300" letter-spacing="1">싫어서 멈춘 게 아니라,</text>
      <text x="540" y="${interpY + 58}" text-anchor="middle" font-family="'Noto Sans KR','Apple SD Gothic Neo',NanumSquare,sans-serif" font-size="34" fill="#F4F8FF" font-weight="300" letter-spacing="1">고르지 못해서 멈춰 있는 거예요.</text>
      <text x="540" y="${interpY + 116}" text-anchor="middle" font-family="'Noto Sans KR','Apple SD Gothic Neo',NanumSquare,sans-serif" font-size="34" fill="#F4F8FF" font-weight="300" letter-spacing="1">재촉보다 안심이 먼저 필요해요.</text>
    </g>
