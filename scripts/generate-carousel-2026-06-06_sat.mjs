 ...
 * 훅: 관계가 곧 달라지기 전에, / 카드가 먼저 보여주는 3가지 결
...
const outputDir = resolve(rootDir, 'content-output/2026-06-06_sat/carousel')

const W = 1080, H = 1350

const PORTRAIT_OVERRIDE = {
  'eight-of-cups': 'public/images/mcards/cups/Eight of Cups.png',
  'two-of-wands': 'public/images/mcards/wands/Two of Wands.png',
  'six-of-swords': 'public/images/mcards/swords/Six of Swords.png',
}
...
async function slide01() {
  ...
  if (masked) {
    ...
    <text x="540" y="1085" font-family="'Noto Sans KR','Apple SD Gothic Neo',NanumSquare,sans-serif" font-size="44" font-weight="300" fill="#F4F8FF" text-anchor="middle">관계가 곧 달라지기 전에,</text>
    <text x="540" y="1145" font-family="'Noto Sans KR','Apple SD Gothic Neo',NanumSquare,sans-serif" font-size="44" font-weight="300" fill="#F4F8FF" text-anchor="middle">카드가 먼저 보여주는</text>
...
async function main() {
  console.log('=== 2026-06-06 carousel (Wheel of Fortune) 이미지 생성 ===')
  mkdirSync(outputDir, { recursive: true })

  await slide01()

  await contentSlide('eight-of-cups',
    'Eight of Cups', '익숙함에서 마음이 멀어질 때',
    '늘 머물던 자리가 어느 순간\n예전 같지 않게 느껴질 때\n\n그 미세한 거리감이\n흐름이 바뀌고 있다는 첫 신호예요\n\n키워드: 떠남 · 거리감 · 다음 단계',
    1, 'slide02.png')

  await contentSlide('two-of-wands',
    'Two of Wands', '다음을 내다보기 시작할 때',
    '지금 자리에 머물던 시선이\n문득 더 넓은 쪽을 바라볼 때\n\n바라보는 방향이 달라지면\n관계의 흐름도 함께 움직이기 시작해요\n\n키워드: 계획 · 새 방향 · 가능성',
    2, 'slide03.png')

  await contentSlide('six-of-swords',
    'Six of Swords', '더 잔잔한 쪽으로 옮겨갈 때',
    '소란하던 마음이 가라앉고\n조용한 쪽으로 방향을 틀 때\n\n그 잔잔함이\n새로운 흐름으로 건너가는 신호예요\n\n키워드: 전환 · 이동 · 나아감',
    3, 'slide04.png')
