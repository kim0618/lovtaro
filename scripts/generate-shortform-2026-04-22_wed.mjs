/**
 * 2026-04-22 수요일 shortform (캐러셀용 릴스) 이미지 생성 (코스믹 스타일)
 * - scene01: 커버 - Wheel of Fortune 프레임 + 하단 훅
 * - scene02-04: 작은 프레임 카드 + 카드명 + 해석
 * - scene05: CTA - Wheel of Fortune + 저장/공유
 *
 * 실행: node scripts/generate-shortform-2026-04-22_wed.mjs
 */
...
const outputDir = resolve(rootDir, 'content-output/2026-04-22_wed/shortform')
...
async function scene01() {
  ...
  const img = await loadCard('wheel-of-fortune', cardW, cardH)
  ...
  const svg = `<svg xmlns="http://www.w3.org/2000/svg" width="${W}" height="${H}">
    ...
    <g filter="url(#softGlow)">
      <text x="540" y="1530" text-anchor="middle" font-family="sans-serif" font-size="48" font-weight="300" fill="#F4F8FF" letter-spacing="3">연애가 정체돼 있다면,</text>
      <text x="540" y="1600" text-anchor="middle" font-family="sans-serif" font-size="48" font-weight="300" fill="#F4F8FF" letter-spacing="3">카드가 전하는 3가지 메시지</text>
    </g>
...
async function scene05() {
  ...
  const svg = `<svg xmlns="http://www.w3.org/2000/svg" width="${W}" height="${H}">
    ...
    <text x="540" y="${nameKrY}" text-anchor="middle" font-family="sans-serif" font-size="38" fill="#F4F8FF" font-weight="300" letter-spacing="5">운명의 수레바퀴</text>
    <text x="540" y="${nameEnY}" text-anchor="middle" font-family="Georgia, serif" font-style="italic" font-size="22" fill="rgba(232,212,139,0.78)">Wheel of Fortune</text>
    ...
    <g filter="url(#softGlow)">
      ...
      <text x="540" y="1435" text-anchor="middle" font-family="sans-serif" font-size="26" fill="rgba(232,212,139,0.78)" letter-spacing="2">마음이 무거울 때 다시 꺼내보세요</text>
    </g>
    <line x1="380" y1="1485" x2="700" y2="1485" stroke="rgba(201,168,76,0.25)" stroke-width="1"/>
    <text x="540" y="1535" text-anchor="middle" font-family="sans-serif" font-size="24" fill="rgba(200,190,240,0.55)">연애가 정체된 친구가 있다면</text>
...
async function main() {
  console.log('=== 2026-04-22 shortform 릴스 (코스믹 스타일) ===')
  mkdirSync(outputDir, { recursive: true })

  await scene01()

  await contentScene('hermit',
    '은둔자', 'The Hermit',
    '잠시 멈출 시간',
    '지금은 밖을 보기보다\n내 마음을 들여다볼 시간이에요\n\n감정이 복잡할수록\n혼자의 시간이 해답을 줘요\n\n키워드: 내면 · 성찰 · 멈춤',
    1, 'scene02.png')
