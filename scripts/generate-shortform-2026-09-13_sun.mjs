 ...
 * scene04: 1번 공개 (The Hermit) - 프레임 카드 + 키워드 + 한줄 훅
 * scene05: 2번 공개 (Nine of Wands) - 동일 구조
 * scene06: 3번 공개 (Two of Pentacles) - 동일 구조
...
const outputDir = resolve(rootDir, 'content-output/2026-09-13_sun/insta')
const W = 1080, H = 1920
...
const CARDS = [
  { num: '①', slug: 'moon', imageSrc: 'public/images/cards-png/moon.png',
    nameKr: '달', nameEn: 'The Moon', keywords: '무의식 · 숨겨진 감정 · 흐림',
    hook: ['주 초반, 서로의 속도가', '아직 흐릿할 수 있어요'] },
  { num: '②', slug: 'two-of-cups', imageSrc: 'public/images/mcards/cups/Two of Cups.png',
    nameKr: '컵의 2', nameEn: 'Two of Cups', keywords: '교감 · 서로를 향함 · 연결',
    hook: ['주 중반, 눈높이가 맞는', '순간이 올 수 있어요'] },
  { num: '③', slug: 'four-of-wands', imageSrc: 'public/images/mcards/wands/Four of Wands.png',
    nameKr: '완드의 4', nameEn: 'Four of Wands', keywords: '안정 · 함께하는 자리 · 안착',
    hook: ['주 후반, 같은 자리에', '나란히 설 수 있어요'] },
]
...
async function scene01() {
  ...
  const svg = `<svg xmlns="http://www.w3.org/2000/svg" width="${W}" height="${H}">
    ...
    <g filter="url(#softGlow)">
      <text x="540" y="420" text-anchor="middle" font-family="${KO_STACK}" font-size="46" fill="#F4F8FF" letter-spacing="1" font-weight="300">다음 주, 따로 가던 두 사람이</text>
      <text x="540" y="505" text-anchor="middle" font-family="${KO_STACK}" font-size="50" fill="#F4F8FF" letter-spacing="2" font-weight="300">나란히 걷게 될까요?</text>
    </g>
...
async function scene07() {
  ...
  const svg = `<svg xmlns="http://www.w3.org/2000/svg" width="${W}" height="${H}">
    ...
    ${cosmicBodyNoMoon(747)}

    <ellipse cx="540" cy="${thumbY + thumbH / 2}" rx="560" ry="${thumbH * 0.7}" fill="url(#thumbGlow)" filter="url(#glowBlur)"/>

    <!-- 메인 CTA: 댓글 유도 (가장 큰 텍스트, 상단) -->
    <g filter="url(#softGlow)">
      <text x="540" y="260" text-anchor="middle" font-family="${KO_STACK}" font-size="56" fill="#F4F8FF" letter-spacing="2" font-weight="300">어떤 카드가 나왔는지</text>
      <text x="540" y="340" text-anchor="middle" font-family="${KO_STACK}" font-size="56" fill="#F4F8FF" letter-spacing="2" font-weight="300">댓글로 남겨두셔도 좋아요</text>
    </g>
...
async function main() {
  console.log('=== 2026-09-13 Sunday Tarot Preview - The Moon/Two of Cups/Four of Wands ===')
  mkdirSync(outputDir, { recursive: true })
  await scene01()                                  // 훅 도입
  await scene02()                                  // 선택 유도
  await revealScene(CARDS[0], 401, 'scene03.png')  // 1번 공개 (The Tower)
  await revealScene(CARDS[1], 407, 'scene04.png')  // 2번 공개 (Eight of Wands)
  await revealScene(CARDS[2], 413, 'scene05.png')  // 3번 공개 (Eight of Pentacles)
