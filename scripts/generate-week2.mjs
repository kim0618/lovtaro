...
const WEEK2 = [
  {
    ...
    theme: '이번 주 연애운',
    cards: [
      { slug: 'fool', name: '바보', nameEn: 'The Fool', day: '월', line: '새로운 시작의 에너지\n용기 내서 먼저 다가가보세요' },
      { slug: 'empress', name: '여황제', nameEn: 'The Empress', day: '화', line: '따뜻한 감정이 흐르는 날\n다정함이 무기가 돼요' },
      { slug: 'hierophant', name: '교황', nameEn: 'The Hierophant', day: '수', line: '진지한 대화가 필요한 날\n관계를 더 깊게 만들 수 있어요' },
      { slug: 'justice', name: '정의', nameEn: 'Justice', day: '목', line: '균형 잡힌 시선이 필요한 날\n감정과 이성 사이를 살펴보세요' },
      { slug: 'hanged-man', name: '매달린 사람', nameEn: 'The Hanged Man', day: '금', line: '기다림이 답이 되는 날\n조급해하지 마세요' },
      { slug: 'temperance', name: '절제', nameEn: 'Temperance', day: '토', line: '천천히 꾸준히 가야 하는 날\n관계의 균형을 찾아보세요' },
      { slug: 'star', name: '별', nameEn: 'The Star', day: '일', line: '희망이 보이는 날\n좋은 흐름이 들어오고 있어요' },
    ],
...
async function carouselCTA(mainText, subText, icon, bgCardSlug) {
  ...
  const svg = carouselBg(`
  ${iconSvg}
  <text x="540" y="500" font-family="sans-serif" font-size="40" font-weight="300" fill="#F4F8FF" text-anchor="middle">${esc(mainText)}</text>
  <text x="540" y="555" font-family="sans-serif" font-size="24" fill="rgba(167,183,214,0.5)" text-anchor="middle">${esc(subText)}</text>
  <line x1="380" y1="620" x2="700" y2="620" stroke="rgba(77,163,255,0.12)" stroke-width="1"/>
  <text x="540" y="670" font-family="sans-serif" font-size="22" fill="rgba(143,211,255,0.5)" text-anchor="middle">무료로 직접 뽑아보기 · lovtaro.kr</text>
  ...
  if (bgCardSlug) {
    ...
    if (bgImg) {
      ...
      base = await sharp(base)
        .composite([{ input: faded, left: (1080 - bgW) / 2, top: 500 }])
        .png().toBuffer()
    }
  }
  return base
...
async function main() {
  ...
  for (const day of WEEK2) {
    ...
    if (day.type === 'carousel') {
      ...
      // 표지
      const cover = await carouselCover('이번 주 연애운, 요일별로 확인하세요', '4/13 - 4/19 이번 주 연애 흐름', '저장해두고 매일 확인하는 이번 주 연애운', day.cards.map(c => c.slug))
      ...
      // 저장+공유 합친 CTA
      const combinedCta = await carouselCTACombined('star')
    ...
    if (day.type === 'carousel-edu') {
      ...
      // 저장+공유 합친 CTA (감성 톤)
      const combinedCta = await carouselCTACombined('fool',
        ['저장해두고', '이 카드가 다시 나오면 꺼내보세요'],
        ['바보 카드가 나온 친구가 있다면', '가볍게 공유해보세요']
      )
    ...
    if (day.type === 'carousel-test') {
      ...
      // 표지
      const cover = await carouselCover('읽씹당하면 당신은?', '유형별 타로 카드 매칭', '댓글에 A/B/C/D로 알려주세요', day.testItems.map(t => t.slug))
      writeFileSync(resolve(cDir, 'slide01.png'), cover)
      ...
      // 댓글+공유 합친 CTA
      const combinedCta = await carouselCTACombined('tower',
...
[폴더 구조]
4월_13일__월_/  캐러셀 10장 + 스토리 + 투표
4월_14일__화_/  릴스 3장면(코스믹) + CapCut가이드 + 스토리 + 질문박스
4월_15일__수_/  캐러셀(교육) 6장 + 스토리 + This or That
4월_16일__목_/  릴스 3장면(코스믹) + CapCut가이드 + 스토리 + 질문답변
4월_17일__금_/  캐러셀(테스트) 8장 + 스토리 + 투표
4월_18일__토_/  릴스 3장면(코스믹, 긴버전) + CapCut가이드 + 스토리 + 질문박스
