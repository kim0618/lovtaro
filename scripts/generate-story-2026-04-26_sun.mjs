/**
 * 2026-04-26 일요일 story 이미지 생성 (마이너 아르카나, story 집중 운영)
 * - story01: Nine of Cups + "소원한 감정이 채워지는 하루"
 *
 * 실행: node scripts/generate-story-2026-04-26_sun.mjs
 */
...
const outputDir = resolve(rootDir, 'content-output/2026-04-26_sun/story')
const W = 1080, H = 1920
...
async function main() {
  console.log('=== 2026-04-26 story 이미지 생성 ===')
  ...
  const img = await loadCard('cups', 'Nine of Cups', cW, cH)
  if (!img) { console.error('❌ Nine of Cups.png 없음'); process.exit(1) }
  ...
  const hookLines = ['소원한 감정이 채워지는 하루', '작은 바람이 조용히 이뤄져요']
  const keywords = ['만족', '감정의 풍요', '소원 성취']
  ...
  <text x="540" y="${engNameY}" font-family="Georgia, serif" font-size="22" font-style="italic" fill="rgba(200,180,140,0.65)" text-anchor="middle">Nine of Cups</text>
  <g filter="url(#tg)">
    ${hookSvg}
  </g>
