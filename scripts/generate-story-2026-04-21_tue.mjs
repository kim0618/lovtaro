/**
 * 2026-04-21 화요일 story 이미지 생성 (마이너 아르카나)
 * - story01: Three of Cups + "마음이 가벼워지는 하루"
 *
 * 실행: node scripts/generate-story-2026-04-21_tue.mjs
 */
...
const outputDir = resolve(rootDir, 'content-output/2026-04-21_tue/story')
const W = 1080, H = 1920
...
async function main() {
  console.log('=== 2026-04-21 story 이미지 생성 ===')
  ...
  const img = await loadCard('cups', 'Three of Cups', cW, cH)
  if (!img) { console.error('❌ Three of Cups.png 없음'); process.exit(1) }
  ...
  const hookLines = ['마음이 가벼워지는 하루', '가까운 사람과 웃음이 늘어나요']
  const keywords = ['우정', '기쁨', '풍성한 감정']
  ...
  <text x="540" y="${engNameY}" font-family="Georgia, serif" font-size="22" font-style="italic" fill="rgba(200,180,140,0.65)" text-anchor="middle">Three of Cups</text>
  <g filter="url(#tg)">
    ${hookSvg}
  </g>
